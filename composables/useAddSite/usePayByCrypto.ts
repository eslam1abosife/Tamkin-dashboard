    ...
    const paywithCrypto = async (hash, price,code) => {
        try {
            const res = await api.post('/Apps/post/AppsInvouceCrypto', {
                ...
                "urls":  addSiteStore.packagePayload.urls.filter((website: any) => website.url !== null).map((website: any) => website.url) ,
                "apps": addSiteStore.packagePayload.apps,
                  "coupon_code": addSiteStore.promo || null,
                  "packageName": addSiteStore.packagePayload.package,
                  "payDateType": addSiteStore.packagePayload.payDateType,//1,3.12
                  "cryptoinput": {
                    "code": code,
                    "hash": hash,
                    "crypto": addSiteStore.selectedCrypto.name,
                    "network": addSiteStore.selectedCrypto.network,
                    "date": new Date(),
                    "amount": price
                  },
