...
<template>
  <div class=" relative">
    ...
    >
      ...
      <div
          v-loading="getAllMembersLoading"
          :style="{
         ...
         divide-gray-200 dark:divide-darkborder">
            ...
            <tr class="" v-for="(member, index) in paginatedFilteredTeamMembers" :key="index">
              ...
              <td class=" text-[14px] font-[400] text-darkGrey dark:text-whiteTamkin">
                ...
                >
                  <button
                      :disabled="member.is_active || isOwner(member) || (reInviteLoading && currEmail === member.member_email)"
                      :class="[member.is_active || isOwner(member)  || (reInviteLoading && currEmail === member.member_email) ? `opacity-40`: 'opacity-100']"
                    ...
                    <svg
                        ...
                        :class="!member.is_active && (!(reInviteLoading && currEmail === member.member_email)) ? `hover:text-tamkin` : null"
                        viewBox="0 0 22 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
