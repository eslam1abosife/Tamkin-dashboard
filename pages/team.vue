...
<div class="mt-[23px]">
    ...
    <div class="mt-[18px] flex lg:space-y-0 space-y-[16px] items-center 
    lg:flex-row flex-col justify-center lg:justify-start lg:space-x-[16px]">
<div class="flex items-center justify-between px-[16px] py-[23px] w-full  bg-white 2xl:w-full lg:w-[388px] h-[108px] rounded-[10px]  border-[1px] border-lightGrey " style="padding:30px, 16px, 30px, 16px;">
    <div class="w-[63px] h-[62px] bg-tamkin rounded-full flex items-center justify-center cursor-pointer" @click="modalStore.controlTeamEditPictureModal">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-[26px] h-[23px] text-white">
    ...
    <div v-if="!editTeamNameMode">
        <h1 class="font-[500] text-[13px] leading-[19.5px] text-darkGrey">Your team name <br>
            <span class=" font-bold">Tamkin</span></h1>
    </div>

    <div v-else>
        <div class=" w-[159px] relative">
            <input type="text" placeholder="{{$t('Your team name')}}" id="teamName" class="input_floating_label peer w-full"
              v-model="v$.teamName.$model" :class="{
          input_error:
            (v$.teamName.$error && v$.teamName.required.$invalid),
          input_success: !v$.teamName.$error && !v$.teamName.$invalid,
        }" />
            <label for="teamName" class="floating_label" :class="[
          (v$.teamName.$error && v$.teamName.required.$invalid)
            ? '!text-error'
            : '',
        ]">
            ...
            <div class="w-full lg:w-4/6 mt-2" v-if="(v$.teamName.$error && v$.teamName.required.$invalid)">
              <p class="error_message">
                <span v-if="v$.teamName.$error && v$.teamName.required.$invalid">{{ $t("teamName_is_required")
                  }}</span>
    ...
    <div v-if="!editTeamNameMode">
        <button @click="editTeamNameMode = !editTeamNameMode" class="btn_bordered_dashboard font-[500] text-[15px] leading-[22.5px]">
            Edit Team
        </button>
    </div>
    <div v-else>
...
<div     :style="{     backgroundImage: `linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.5)), url(${banner})`, 
}"

 style="box-shadow: 0px 4px 24px 8px #51459F14;padding:30px, 16px, 30px, 16px; ;
  background-size:cover;" class="b 2xl:w-full lg:w-[388px] w-full  flex items-center justify-between space-x-[30px] px-[16px] py-[23px] bg-white h-[108px] rounded-[10px]  border-[1px] border-lightGrey " >
  ...
  <div class="flex flex-col items-center justify-center text-darkGrey" >
    <div class=""><h1 class="font-[600] text-[15px] lg:leading-[22.5px] text-center">Total Member</h1></div>
    <div class=""><h1 class="font-[500] text-[15px] leading-[22.5px] ">4</h1></div>
</div>
<div class="bg-white w-[63px] h-[48px]  border-[0.5px] border-[#48D916] rounded-[9px] p-[10px] text-[#48D916] flex flex-col items-center justify-center">
<div>
...
<div class="bg-white w-[63px]   h-[48px] border-[0.5px] border-[#EA4335] rounded-[9px] p-[10px] text-[#EA4335] flex flex-col items-center justify-center">
    <div>
          ...
          <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            ...
            >
              <div class=" rounded-[10px] bg-white">
                <div class="flex items-center justify-center lg:justify-between lg:flex-row flex-col">
                    <div class="">
                      <div
                  ...
                  <div class="flex items-center justify-between lg:p-[16px]">
                      <div class="p-[16px] search_input w-[275px]">
                        ...
                        <div class="w-[141px] ">
                         <button class="btn-dashboard" @click="modalStore.controlInviteMemberModal">
                          Invite Member
                         </button>
                        </div>
                  </div>
                ...
                <thead>
                  ...
                  <tr>
                    <th class="py-3.5 px-4 text-left text-[14px] font-[600] text-darkGrey">Name</th>
                    <th class="py-3.5 px-4 text-left text-[14px] font-[600] text-darkGrey">Email</th>
                    <th class="py-3.5 px-4 text-left text-[14px] font-[600] text-darkGrey">Permissions</th>
                    <th class="py-3.5 px-4 text-left text-[14px] font-[600] text-darkGrey">Action</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr>
                      <td class="flex items-center lg:flex-row flex-col justify-start lg:px-4 lg:py-4 lg:space-y-[0] space-y-[10px] lg:space-x-4 text-[14px] font-[400] text-darkGrey">
                        <img src="/assets/imgs/icons/avatar_table.svg" alt="" class="lg:h-full h-[30px] lg:mt-0 mt-[4px]">
                        <div class="lg:order-1 order-2 lg:py-0 py-1">Ali Ahmed</div>
                        <div class="order-1 flex items-center justify-center text-white text-[10px] font-[500] leading-[15px] w-[47px] h-[23px] rounded-[17px] p-[10px]" style="background: linear-gradient(180deg, #2DADA3 0%, #71DAD2 100%);">
                          Owner
                        </div>
                      </td>
                      <td class="px-4 py-4 text-[14px] font-[400] text-darkGrey">Ali Ahmed@gmail.com</td>
                      <td class="px-4 py-4 text-[14px] font-[400] text-darkGrey">
                        <div class="flex items-center ">
                          <button @click="modalStore.controlEditPermissionsModal" class="flex items-center space-x-2 bg-transparent underline focus:outline-none">
                            <div>
                                Permissions
                            </div>
