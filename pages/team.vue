...
const currEmail = ref(null);
const invitedUsers = ref([]);
const countdown = ref(29); // countdown timer in seconds
let intervalId = null;

const removeAfter29Sec = (emailToRemove) => {
...
const reinviteUser = async (email: string) => {
  ...
  try {
    ...
    countdown.value = 29;
    invitedUsers.value.push(currEmail.value);
    removeAfter29Sec(currEmail.value);
  } catch (err) {
  } finally {
...
const doRenameTeam = async () => {
  ...
  try {
    await renameTeam(state.teamName);
    getCurrTeam();
    editTeamNameMode.value = false;
    renamedSuccessfullyToast.value = true;
    setTimeout(() => {
      renamedSuccessfullyToast.value = false;
    }, 2000);
  } catch (err) {
    console.error(err);
  }
...
<template>
 ...
 />
    ...
    <div
      ...
      <div v-loading="getCurrTeamLoading"
          ...
          <div @click="openModal('editteampic', 'team')" v-if="!currTeam?.team_image">
            <div
              ...
              <svg width="27" height="24" viewBox="0 0 27 24"
                class="lg:w-[32px] lg:h-[32px] w-[15px] h-[15px] ipad-max:w-[15px] ipad-max:h-[15px]" fill="none"
              ...
              <div class=" ">
                ...
                <div
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-[10px] h-[10px] text-[#021328] dark:text-whiteTamkin">
      ...
      v$.teamName.$error && v$.teamName.required.$invalid
        ...
        : '',
              ...
              <div class="w-full lg:w-4/6" v-if="v$.teamName.$error && v$.teamName.required.$invalid">
        ...
        <div class="flex items-center justify-center flex-shrink-0">
          <div v-if="!editTeamNameMode">
            <button @click="() => (editTeamNameMode = !editTeamNameMode)"
    ...
    <section class="mx-auto mt-[24px]" v-loading="getAllMembersLoading">
      <div
        ...
        <div class="flex items-center justify-between lg:flex-nowrap flex-wrap w-full">
          <div class="p-[16px]">
            <div class="text-[16px] font-[600] py-[24px] text-[#021328] dark:text-whiteTamkin"
          ...
          <div class="flex items-center justify-between lg:justify-evenly px-[16px] space-x-[10px]">
            ...
            <div class="lg:w-[250px] w-2/4">
              <button class="btn-dashboard hover_tamkin" @click="openModal('invitemember')">
                Invite Member
              </button>
            </div>
          </div>
      ...
      " :src="myUser.photoURL" class="lg:h-full h-[30px] hidden lg:block md:hidden h-8 w-8 rounded-full" />
        ...
        member.first_name + " " + member.last_name
                ...
                <td
                  ...
                  <div class="flex items-center justify-start">
                    <button :disabled="isOwner(member)" @click="openPermissions(member)" :class="isOwner(member) ?
      'opacity-40'
      : 'opacity-100'
      ...
      <div class="flex flex-col lg:flex-row md:flex-row justify-between items-center py-[16px]"
        ...
        <div class="flex items-center rtl:space-x-reverse space-x-2 mb-4 lg:mb-0">
          ...
          <div class="flex space-x-2 rtl:space-x-reverse">
            <button v-for="option in perPageOptions" :key="option" :style="perPage === option
      ...
      perPage === option ? '' : 'bg-[#A7A7A7] hover:bg-lightGrey',
        ...
        <div class="flex items-center rtl:space-x-reverse space-x-2">
          ...
          <div v-if="teamMembers" class="flex space-x-2 rtl:space-x-reverse">
            <button v-for="i in totalPages" :key="i" :style="currentPage === i
      ? 'background: linear-gradient(180deg, #2dada3 0%, #71dad2 100%);'
      : ''
      " :class="[
