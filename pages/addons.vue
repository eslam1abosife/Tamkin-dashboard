...
<template>
  <div class="relative h-full w-full">
    <LazyModalsConfirm :showModal="checkboxStore.routeLeaveModal" title="Save  your changes"
      sub-title="Do you want to save the changes before moving on?" confirm-btn-type="other"
    ...
    <div class="w-full h-full relative">
      ...
      <div class="mt-[64px] md:mt-[94px] bg-white dark:bg-tamkinDarkPrimary rounded-[10px] xs:px-0 px-[15px] pb-[24px] shadow-md 
      -shadow-y-[1px] relative">
        <div class="flex items-center justify-start  pt-[24px] xs:px-[15px]">
          <div>
            <h1 class="text-[14px] xs:text-[12px] lg:text-[18px] font-[500] leading-[30px] dark:text-whiteTamkin ">
              Adjust the Main Menu</h1>
          </div>

          <div @click.stop="collapseMenu('adjustMenu')"
            v-on-click-outside="() => collapseStore.removeMenu('adjustMenu')" :class="[
                ...
                <div>
                  <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M11.2241 13.063L6.5 17" class="dark:!stroke-white stroke-darkGrey" stroke-linecap="round"
                      stroke-linejoin="round" />
                    <path d="M10.2793 17.0002H6.5V13.8506" class="dark:!stroke-white stroke-darkGrey"
                      stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </div>
                <div class="text_mini">
              ...
              <div class="arrow">
                <svg width="16" class="" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path
                    ...
                    class="fill-white dark:!fill-tamkinDarkPrimary" filter="url(#shadow-sm)" />
                </svg>
              </div>
            </div>
          </div>
        ...
        <div class="flex flex-col items-start justify-center  mt-[18px] divide-y pb-[16px]"
          v-if="!collapseStore.collapses.includes('adjustMenu')">

          <draggable v-model="checkboxStore.AdjustMainMenuCards"
            @change="checkboxStore.onDragChange('AdjustMainMenuCards', 'initialCardsOrder')"
            @start="checkboxStore.onDragStart('initialCardsOrder')"
            @end="checkboxStore.onDragEnd('AdjustMainMenuCards', 'initialCardsOrder')" item-key="name" class="w-full"
            handle=".handle">
            ...
            dark:bg-darkSecondary">
                <div class="flex items-center justify-start rtl:space-x-reverse xs:space-x-[4px] space-x-[13px] w-full">
                  <img src="/assets/imgs/addons/left_item.svg"
                    :class="[!isChecked(element.checkboxId) ? 'opacity-60' : '']"
                    class="cursor-pointer handle w-[8px] h-[20px] lg:w-[11px] lg:h-[25px]" />

                  <img :src="getImagePath(element.icon)" class="lg:w-[45px] lg:h-[46px] w-[30px] h-[30px]"
                    :class="[!isChecked(element.checkboxId) ? 'opacity-60' : '']" />
                ...
                lg:leading-[16.39px]">
                  ...
                  <div class="ml-auto">
                    <label :for="element.checkboxId" class="toggle_wrap">
                      <input type="checkbox" :id="element.checkboxId" class="sr-only"
                        :checked="isChecked(element.checkboxId)" @change="toggleCheckbox(element.checkboxId)" />
                      <div class="toggle_parent" :class="[
                        isChecked(element.checkboxId)
                          ? 'active'
                          : 'in_active',
                      ]">
                        <div class="toggle_inner" :class="{ 'active': isChecked(element.checkboxId) }">
                          <img v-if="isChecked(element.checkboxId)" src="/assets/imgs/addons/active_toggle.svg"
                            class="w-[28px] h-[28px]" />
        ...
        <div v-else class="text-[14px] leading-[24px] font-[400] text-[#585B5B] pt-[6px] dark:text-whiteTamkin ">
          Temporibus rerum vel laudantium. Earum velit qui quis quia autem iusto est veritatis dolore. Exercitationem et
          omnis
          ea quidem
      ...
      rounded-[10px] xs:px-0 px-[15px] pb-[24px] shadow-md -shadow-y-[1px] relative">
        <div class="flex items-center justify-start pt-[24px] ">
          ...
          <div @click.stop="collapseStore.collapseMenu('ManageMenu')"
            v-on-click-outside="() => collapseStore.removeMenu('ManageMenu')" :class="[
        ...
        <div class="flex flex-col items-start justify-center pb-[16px] mt-[18px] divide-y"
          v-if="!collapseStore.collapses.includes('ManageCard')">

          <draggable v-model="checkboxStore.manageProfileCards"
            ...
            @end="checkboxStore.onDragEnd('manageProfileCards', 'initialManageProfileCards')" item-key="name"
            class="w-full" handle=".handle">
            <template #item="{ element }">
              <div
      ...
      <div class="mt-[30px] bg-white dark:bg-tamkinDarkPrimary rounded-[10px] pb-[24px] mb-[80px]
       shadow-md -shadow-y-[1px] relative">
        <div class="flex items-center justify-start ltr:ml-[15px] rtl:mr-[15px] pt-[24px]">
          <div>
            <h1 class="text-[14px] xs:text-[12px] lg:text-[18px] font-[500] leading-[30px] dark:text-whiteTamkin">Live
              Translation</h1>
          ...
          <div @click.stop="collapseStore.collapseMenu('LiveTranslationAddons')"
            v-on-click-outside="() => collapseStore.removeMenu('LiveTranslationAddons')" :class="[
            ...
            <div v-if="menus.includes('LiveTranslationAddons')" class="mini_SizeMenu divide-y">
              ...
              <div class="mini_wrap" @click="collapseStore.collapseCard('LiveTranslationAddonsCard')">
                <div>
                  <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M13.7754 10.937L18.4995 7" class="dark:!stroke-white stroke-darkGrey"
                      stroke-linecap="round" stroke-linejoin="round" />
        ...
        <div class="flex items-center justify-center lg:justify-between lg:flex-nowrap 
        flex-wrap lg:px-[25px] mt-[64px] w-full lg:space-y-0 space-y-10 md:space-y-0 
        md:space-x-10 md:flex-nowrap ipad-max:space-x-10 lg:space-x-24 2xl:space-x-44 md:px-[25px]" v-if="
          horizontalView &&
          !collapseStore.collapses.includes('LiveTranslationAddonsCard')
        ">
          <div class="w-full max-w-[270px] flex flex-col items-center justify-start h-[267px] 
          relative custom-border rounded-big rounded-[19px] hover:bg-selected dark:hover:bg-p">
            ...
            <div class="flex items-center justify-evenly mt-[12px] rtl:space-x-reverse space-x-[6px]">
              ...
              <div class="text-[12px] leading-[14.16px] font-[500] text-black dark:text-whiteTamkin">Live translation
              </div>
            </div>
            <button
              class="btn_bordered_dashboard mt-[24px] !text-darkGrey dark:!text-whiteTamkin hover:!text-white">Upgrade
              Now</button>
          ...
          <div class="w-full max-w-[270px] flex flex-col items-center  justify-start h-[267px] relative custom-border 
          ...
          <div class="w-full max-w-[270px] flex flex-col items-center justify-start h-[267px] relative
        ...
        <div
          ...
          <div 
            ...
            <div class="flex flex-col items-start justify-center py-[14px] mx-[15px]">
              <div class="text-[14px] lg:text-[20px] font-[600] text-[#021328] dark:text-whiteTamkin">
                ${{ annual_prices ? 1200 : "100.00"
                }}<span class="text-[13px]">/{{ annual_prices ? "year" : "mo" }}</span>
              </div>
              <div class="text-[12px] lg:text-[14px] whitespace-nowrap font-[500] text-[#585B5B] dark:text-whiteTamkin/90">
                For 1 million characters
              </div>
            </div>

            <button class="rtl:mr-auto ltr:ml-auto btn_bordered_dashboard lg:w-1/6 ipad-max:w-1/4  mt-[24px]  md:w-1/4 w-2/4
            hover:!text-white my-[19px] rtl:ml-[15px] ltr:mr-[15px] !p-1">
              Upgrade Now
            </button>
          </div>

          <div 
            class="flex items-center justify-start h-[77px] w-full bg-selected dark:bg-p_secondary relative custom-border 
            rounded-big rounded-[19px]   ">
            <div class="ltr:ml-[15px] rtl:mr-[15px]">
              <img src="/assets/imgs/addons/live_vertical.svg" />
            </div>
            ...
            <button class="rtl:mr-auto ltr:ml-auto btn-dashboard hover_tamkin  lg:w-1/6 ipad-max:w-1/4  mt-[24px]  md:w-1/4  w-2/4
