...
watch(eventCounter, async () => {
  ...
  if (lastEventCall.value === "deleteApp") {
    ...
    const currentIndex = appList.value.findIndex(app => app.title === currAppName.value.title);
...
const appList = computed(() => {
  ...
  const titleForSearch = currentTab.value === 'internal'
    ? t(ele.title)
    : translatedTitle;
...
const packagemodal = await getPackage(app.package.find(k=>k.name === pack).name)
      ...
      await mysiteStore.setCurrentPackage({...packagemodal.package,
        package_price_role:packagemodal.price_roles,
        billing_duration:app.package.find(k=>k.name === pack).billing_duration,status:new Date() > new Date( app.package.find(k=>k.name === pack).endpackage) ? 'Expired' : app.package.find(k=>k.name === pack).status});
...
const packagemodal = await getPackage(app.package.find(k=>k.name === pack).name)
      ...
      await mysiteStore.setCurrentPackage({...packagemodal.package,package_price_role:packagemodal.price_roles,billing_duration:app.package.find(k=>k.name === pack).billing_duration});
...
<MySiteNopackagebuy v-if="isOpen('upgrade_no_package')"/>
</transition>
    <transition :name="locale === 'ar' ? 'slide-left' : 'slide-right'" mode="out-in">
<MySiteUpgrade  v-if="isOpen('upgrade_mysite_package')"/>
</transition>
    <transition :name="locale === 'ar' ? 'slide-left' : 'slide-right'" mode="out-in">
    ...
    >
      ...
      <div>
        <button @click="$router.push(localePath('/packages'))"
    ...
    <div
      ...
      class="ipad-max:mt-[24px] mt-[44px] flex lg:space-y-0 space-y-[16px]
       items-center lg:flex-row flex-col justify-center lg:justify-start w-full rtl:space-x-reverse space-x-[24px]" 
    >
      <div
        class="flex items-center lg:flex-row flex-col justify-start px-[16px] py-[23px] w-full
         bg-white dark:bg-tamkinDarkPrimary md:h-[170px] h-[150px] lg:h-[129px] rounded-[10px]"
            ...
            >
              ...
              <div class="flex items-center rtl:space-x-reverse space-x-[16px]">
                ...
                <div>
                  ...
                  <a 
                  :class="[defaultApp?.title === 'Internal Service' ? '!text-darkGrey/40 cursor-not-allowed' : '']"
                    :href="defaultApp?.title === 'Internal Service' ? '#':defaultApp ? formatToUrl(defaultApp.app_domain) : ''"
                    :target="defaultApp?.title === 'Internal Service' ? '':'_blank'"
                    class="text-tamkin font-[500] text-[14px] leading-[24px] flex"
                    >{{ $t("Visit Site") }}
                    <svg data-slot="icon" class="size-6 ltr:ml-[14px] rtl:mr-[14px]" fill="none" stroke-width="1.5"
                    :class="[defaultApp?.title === 'Internal Service' ? '!text-darkGrey/40 cursor-not-allowed' : '!text-tamkinStart']"
      ...
      <div   style="box-shadow: 0px 4px 24px 8px #51459f1a" class="relative h-[129px] w-full
       bg-white rounded-[10px] flex items-center justify-start">
...
<div class="absolute top-0 rtl:left-[-26px] ltr:right-[-26px]">
  <img src="/imgs/mysite.svg" alt="" class="h-full w-[210px] rtl:scale-x-[-1]" />
   ...
   <div  v-if="getSitesLoading" class=" ipad-max:mt-[24px] mt-[44px] flex lg:space-y-0 space-y-[16px]
       items-center lg:flex-row flex-col justify-center lg:justify-start w-full rtl:space-x-reverse space-x-[24px]">
  
      <!-- Placeholder Container -->
      <div
        class="flex  animate-pulse items-center lg:flex-row flex-col justify-start 
        px-[16px] py-[23px] bg-white dark:bg-gray-700 w-full md:h-[170px] h-[150px] lg:h-[129px] rounded-[10px]"
        style="box-shadow: 0px 4px 24px 8px #51459f1a"
      >
        <!-- Placeholder Content -->
        <div class="w-full space-y-[16px]">
    ...
    <div style="box-shadow: 0px 4px 24px 8px #51459f1a" class="relative h-[129px] w-full bg-white 
    rounded-[10px] flex items-center justify-start animate-pulse ">
      <div class="flex flex-col space-y-[4px] items-start justify-center p-[16px]">
        
        <!-- Placeholder for "My Sites" -->
        <div class="h-[30px] w-full  relative py-[4.5px] to-white flex items-center justify-start  space-x-6">
          <div class="h-[21px] w-full bg-gray-200 rounded"></div> <!-- Placeholder for text -->
          <div class="h-[21px] w-[30px] bg-gray-200 rounded absolute rtl:right-28 ltr:left-24"></div> <!-- Placeholder for number -->
        </div>
        ...
        <div class="h-[30px] w-full  relative py-[4.5px] to-white flex items-center justify-start space-x-6">
          <div class="h-[21px] w-[100px] bg-gray-200 rounded"></div> <!-- Placeholder for text -->
          <div class="h-[21px] w-[30px] bg-gray-200 rounded absolute rtl:right-28 ltr:left-24"></div> <!-- Placeholder for number -->
        </div>
      </div>
              ...
              >
                <thead>
                  <tr class="h-[50px]">
                    ...
                    <th>
                      <div
                        class="flex items-center lg:px-0 px-[100px] justify-start rtl:space-x-reverse 
                        space-x-[6px] lg:text-[14px] font-[600] lg:leading-[21px] text-[12px] leading-[12px]
                    ...
                    <th
                      class=" lg:text-[14px] text-center 
                      font-[600] lg:leading-[21px] text-[12px] ltr:pr-[50px] leading-[12px] text-darkGrey dark:text-whiteTamkin"
                    >
                      {{ $t("Action") }}
                    </th>
                  </tr>
    ...
    :key="index">
      ...
      selectedApp && selectedApp.name === app.name? 'border-[1px] drop-shadow-md !border-tamkinStart':'border-[1px] ']"
    >
      <td class="w-[25%]">
        <div
        ...
        >
          ...
          >
          <div class="flex items-center justify-center " v-if="app.favicon">
            <img 
            :src="app.favicon"
...
{{ getAvatarLetters(app?.title) }}
      ...
      <td
        class=" lg:px-0 w-[150px] text-[12px] lg:text-[14px] ltr:text-left rtl:text-right 
         font-[400] text-darkGrey dark:text-whiteTamkin"
      >
        {{ app.package.length ? $t(capitalizeFirstLetter(app.package[0].billing_duration)) : '-' }}
      </td>
      <td
      ...
      >
        ...
        >
          ...
          <div class="inline-block align-middle rtl:mr-2 ltr:ml-2">
            {{ app.package[0] && app.package[0].title ? $t(`${app.package[0].title}`) : 'Draft website' }}
          </div>
        </div>
 ...
 <div v-if="app.package.length > 0">
  <div
  v-if="app.package[0]&&  new Date() > new Date(app.package[0].endpackage)"
...
{{ $t(`${app.package[0].status}`) }}
  ...
  class="bg-gradient-to-r from-orange-600 to-orange-400 rounded-[17px]  flex items-center justify-center 
        h-[25px] max-w-[150px] w-[100px] text-white text-[12px] leading-[18px]"
>
  {{ app.package[0].status === 'Pending'? $t('Under Review') : $t(`${app.package[0].status}`) }}
</div>
<div
@click="$router.push(localePath('/embed-code'))"
v-if="(app.package && app.package.length > 0 && app.package[0].status === 'not_installed' && new Date() < new Date(app.package[0].endpackage)) "
...
{{ $t("Not installed") }}
 ...
 <div class="rtl:text-right ltr:text-left" v-else>-</div>
      ...
      >
        ...
        <button 
         ...
         class="disabled:opacity-40  disabled:cursor-not-allowed flex items-center justify-center hover:opacity-50 w-6 h-6"
          @click="getPackageAndOpenPaymenModal(app,(app.package.length ? app.package[0].name : null))">
          <img src="/assets/imgs/installed.svg" v-if="!loadingBlock.find(a=>(app.package.length ? a.pack === app.package[0].name : true) && a.app === app)"/>
          
  <svg v-if="loadingBlock.find(a=>(app.package.length ? a.pack === app.package[0].name : true) && a.app === app)" 
    class="animate-spin h-5 w-5 text-tamkin" xmlns="http://www.w3.org/2000/svg"
  ...
  <path class="opacity-75" fill="currentColor"
    d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
          ...
          <button  :disabled="app.package.length  ===  1 ||   app.package.length ===0 " @click.prevent="()=>{
 ...
 v-for="(pack,i) in selectedApp.package.slice(1)" :key='i'
    >
      <td class="w-[25%]">
        <div
          class="relative h-[50px] flex items-center justify-start  
       ipad-max:ltr:pl-[0px] ltr:pl-[18px] lg:ltr:pl-[18px] rtl:pr-[18px] lg:mt-0 mt-[20px] text-[14px] font-[400] text-darkGrey dark:text-whiteTamkin"
        >
          <a
            href="#"
            class="gap-3 h-[50px] flex items-center justify-start"
          >
          <div class="flex items-center justify-center " v-if="selectedApp.favicon">
            <img 
...
{{ getAvatarLetters(selectedApp?.title) }}
      ...
      >
        {{ selectedApp.package.length ? $t(capitalizeFirstLetter(pack.billing_duration)) : '-' }}
      </td>
      <td
 ...
 <div v-if="selectedApp.package.length > 0">
  <div
  v-if="pack.endpackage &&  new Date() > new Date(pack.endpackage)"
...
<div
@click="$router.push(localePath('/embed-code'))"
v-if="(pack && pack.status === 'not_installed' && new Date() < new Date(pack.endpackage)) "
class="cursor-pointer  text-[#DE4134] ltr:text-left rtl:text-right text-[14px] font-[500] leading-[21px]  underline"
>
{{ $t("Not installed") }}
</div>
 ...
 <div class="text-center" v-else>-</div>
      ...
      >
        ...
        <button 
         ...
         class="disabled:opacity-40  disabled:cursor-not-allowed flex items-center justify-center hover:opacity-50 w-6 h-6"
          @click="getPackageAndOpenPaymenModal(selectedApp,pack ? pack.name : null)">
          <img src="/assets/imgs/installed.svg" v-if="!loadingBlock.find(a=>(pack ? a.pack === pack.name : true) && a.app === selectedApp)"/>
          
  <svg v-if="loadingBlock.find(a=>(pack ? a.pack === pack.name : true)&& a.app === selectedApp)" 
    class="animate-spin h-5 w-5 text-tamkin" xmlns="http://www.w3.org/2000/svg"
  fill="none" viewBox="0 0 24 24">
  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
  <path class="opacity-75" fill="currentColor"
 ...
 <td
  ...
  />
    ...
    >
      ...
      {{ pk.billing_duration ? $t(capitalizeFirstLetter(pk.billing_duration)) : $t('Free - Monthly') }}
    </td>
    <td
    ...
    >
      {{ pk.status === 'Pending' || pk.status === 'Pendding'? $t('Under Review') : $t(`${pk.status}`) }}
    </div>
    <div
    ...
    >
      {{ pk.from_date ? new Date(pk.from_date).toDateString() : new Date(pk.creation).toDateString() }}
    </td>
    ...
    >
      ...
      <button
        :disabled="  loadingBlock.find(a=>(pk ? a.pack === pk.name : true) && a.app === internalServiceApp) ||
         pk.billing_duration === 'yearly'  || pk.status === 'Pendding' "
       class="disabled:opacity-40  disabled:cursor-not-allowed flex items-center justify-center hover:opacity-50 h-6 w-6"
        @click="getPackageAndOpenPaymenModal(internalServiceApp,pk ? pk.name : null)">
        
        <img src="/assets/imgs/installed.svg" v-if="!loadingBlock.find(a=>(pk ? a.pack === pk.name : true) && a.app === internalServiceApp)"/> 
...
<path class="opacity-75" fill="currentColor"
  d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
        ...
        <button :disabled="pk.title == 'Free'" @click="openDeleteMember(internalServiceApp)" 
