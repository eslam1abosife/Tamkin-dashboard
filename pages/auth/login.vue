...
<template>
    <div class="flex items-start justify-center flex-col w-full">
      <div class="mx-auto text-center mt-[43px]">
        <div>
        ...
        <h3 class="text-[15px] lg:text-[20px] font-[600] text-secondary_text" style="line-height: 48px;">
          {{ $t("new_to_tamkin") }}
          <a href="" class="text-tamkin underline brightness-[0.8]">{{ $t("get_started") }}</a>
        </h3>
        <button style="line-height: 30px;" class="mt-[10px] transition-all ease-in-out hover:bg-tamkin/15 border-[1.5px] border-tamkin rounded-[10px] bg-transparent h-[55px] w-full">
          <div class="flex items-center justify-center space-x-[16px] lg:space-x-[8px]">
            <div class="font-[600] text-[20px]">{{ $t("login_with_google") }}</div>
        ...
        <div class="space-y-[24px]">
          ...
          <div class="space-y-[24px] w-full">
            ...
            <div class="flex items-center justify-between  ">
              <div >
                <label for="remember_me" class="w-[121px] text-neutral-400 text-[15px] font-medium font-['Poppins'] leading-snug px-1">
                    <input type="checkbox" class="border-[1px] border-lightGrey bg-whiteTamkin rounded-[4px] h-[22px] w-[22px] text-tamkin ring-0 focus:ring-0" id="remember_me" checked />
            ...
            <div class="pt-[40px] w-full">
              <button class="btn-grad-action" @click="loginUser" v-if="!loading" :disabled="v$.email.$invalid || v$.password.$invalid || loading">
                {{ $t("login") }}
              </button>
            </div>
          </div>
