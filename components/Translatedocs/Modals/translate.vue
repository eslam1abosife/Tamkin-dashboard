...
<template>
  <div 
  class="fixed z-[9999] top-0 lg:top-[10%] 2xl:top-[5%] ipad-max:top-[5%] px-[15px] bg-white dark:bg-tamkinDarkPrimary rounded-[10px] 
   lg:w-[640px]  w-full  overflow-y-auto pb-[16px] lg:h-auto lg:max-h-full h-auto max-h-[100vh] 
    lg:overflow-y-visible"
  style="left: 50%; transform: translate(-50%, 0)"
>
  <div style="box-shadow: 1px 0px 20.5px 0px #71dad2bd" class="close_btn " @click="closeModal('translate_'+(translateType === 'PDF Documents' ?'pdf_documents' :'word_documents'))">
    <svg
      class="w-[12px] h-[12px]"
      width="14"
      height="13"
  ...
  <h1 class="text-left mt-[16px] font-[600] text-darkGrey dark:text-whiteTamkin text-[16px] ">
  ...
  <div v-if="!rendering && !failedRender" class="flex flex-col items-start justify-center px-[12px] h-full
   dark:bg-tamkinDarkPrimary w-full mx-auto rounded-[10px] mt-[16px] ">
    <div class="w-full">
      <div  v-if="!v$.documentLink.$model" v-bind="getRootProps()" class="w-full h-auto  p-[10px] rounded-[10px] border-[1px] border-dashed 
       ...
       justify-center flex-col space-y-[10px] ">
        ...
        space-y-[10px] md:space-y-0 space-x-[16px]" v-if="acceptedFilesRef.length > 0">
          <div v-for="file in acceptedFilesRef" :key="file.name" class="rounded-[10px] w-full md:w-auto">
            <div class="flex items-center justify-start w-full space-x-[14px]">
              <div class="relative " >
                ...
                <div v-if="translateType === 'PDF Documents'"  class="w-[78px] h-[78px] flex items-center justify-center ">
                  <img src="/assets/imgs/translatedocs/pdf.png" class="w-full h-[70px]" @click.stop />
                 </div>
                 <div v-if="translateType === 'Word Documents'"  class="w-[78px] h-[78px] flex items-center justify-center  ">
                    <img src="/assets/imgs/translatedocs/word.png" class="w-[70px] h-[70px] rounded-[7px]" @click.stop />
                   </div>
               
              </div>
              <div class="flex flex-col items-start justify-between space-y-[8px] mb-[10px]">
          ...
          <div class="w-auto  md:w-auto  ">
            <button class="text-red-500 hover:bg-[#FFF3F2] hover:border-[#FACECB] w-[32px] h-[32px] mb-[32px]
            border rounded-lg flex items-center justify-center" @click.stop="removeFile(acceptedFilesRef[0])">
        ...
        <div class="w-full mx-auto mt-4" v-if="acceptedFilesRef.length > 0">
          <div class="relative pt-1 flex items-center justify-between">
            <div class="overflow-hidden h-2 w-full text-xs flex rounded bg-[#D7DADA]">
              <div :style="{ width: dynamicWidth + '%' }" class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-gradient-to-r from-tamkinStart to-tamkinEnd rounded"></div>
            </div>
            <div class="ml-2">
      ...
      <div v-if="acceptedFilesRef.length === 0" class="text-[13px] font-[600] leading-[19px] text-darkGrey text-center mt-[8px]" 
  >
        OR
      </div>
      <div v-if="v$.documentLink.$model" class="w-full h-auto  rounded-[10px] border-[1px] p-[10px]   border-dashed 
      border-[#C8CFEB] hover:bg-tamkin-primary hover:bg-opacity-10 dark:border-[#333333] flex items-start
       justify-center flex-col space-y-[10px]  ">
        <div class="flex items-center justify-between w-full  ">
          <div class="relative flex" >
            <img src="/assets/imgs/translatedocs/pdf.png" class="w-full  h-[70px] rounded-[7px] mr-[14px]" @click.stop />
      ...
      <div class="w-full relative mt-[8px] " v-if="acceptedFilesRef.length === 0">
        <input type="text" placeholder="characterName" id="characterName" class="input_floating_label peer w-full"
          v-model="v$.documentLink.$model" :class="{
        ...
        Google Drive , Choose from Dropbox
        </label>
        <div class="w-full lg:w-4/6 " v-if="(v$.documentLink.$error && v$.documentLink.required.$invalid)">
          <p class="error_message">
            <span v-if="v$.documentLink.$error && v$.documentLink.required.$invalid">{{ $t("Document Link is Required")
              }}</span>
          </p>
        </div>
        </div>
