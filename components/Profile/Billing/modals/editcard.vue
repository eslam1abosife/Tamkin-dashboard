  ...
  >
    ...
    <div class="flex flex-col items-start justify-center mt-[21px] w-full">
      ...
      <div class="flex items-center justify-between w-full mb-[20px]">
        ...
        <div class="ml-auto">
          <button
            :disabled="!billingStore.card.is_active || billingStore.card.is_primary"
            :class="
              !billingStore.card.is_active || billingStore.card.is_primary
                ? 'text-lightGrey cursor-not-allowed'
                : ''
            "
            @click="
              navigateTo('edit_card_billing_profile', 'billing', 'deleteModal_card')
            "
            ...
            :title="
              !billingStore.card.is_active || billingStore.card.is_primary
                ? 'This action is not allowed Because this card is primary '
                : ''
            "
          >
