...
<template>
  ...
  >
    <div class="flex items-start justify-start w-full">
      <div class="flex items-center justify-start gap-4 w-full">
        <div class="relative">
          ...
          <img
            v-if="profileStore.member.user_image"
          ...
          />
          <div
            v-else-if="!profileStore.member.user_image && !profileStore.member.first_name && !profileStore.member.last_name"
            class="user-img__skeleton animate-pulse flex space-x-4"
          >
            <div class="rounded-full bg-gray-400 h-[60px] w-[60px] ipad-max:w-[50px] ipad-max:h-[50px] lg:h-[80px] lg:w-[80px]"></div>
          </div>
          <div
            v-else
            class="avatar_img h-[60px] w-[60px] lg:h-[80px] lg:w-[80px] ipad-max:w-[50px] ipad-max:h-[50px]  rounded-full bg-[#2dada3] text-white grid place-content-center select-none"
          >
            <span>
              {{
                getAvatarLetters(
    ...
    >
      <div class="text-[11px] lg:text-[12px] font-[500] ipad-max:text-[10px]"  v-if="profileStore.member.is_active">Active</div>
      <div class="text-[11px] lg:text-[12px] font-[500]" v-else>InActive</div>
    </div>
    </div>
    </div>
  </div>
