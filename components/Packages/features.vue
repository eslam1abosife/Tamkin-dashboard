  ...
  return packagesStore.packages
    .filter(
      (pkg) =>
        ...
        pkg.package_price_role.some((item) => item.title === packagesStore.traffic_level)
    )
    .map((pkg) => {
...
const normalpackeges = packagesStore
  ...
  const hasAccessNames = addonsSection.filter(v => accessNames.includes(v.parent)).length > 0 ;
...
<div class="w-full relative" v-if="compareg">
  <div v-if="!showAllFeatures" class="bg-gradient-to-t  
 absolute inset-auto h-full w-full z-[40]" 
  
  :class="[currentPage === 'sign' ? 'from-[#EEF5FF] via-[#EEF5FF]/30 ' : 'from-white via-white/30 ']">

  </div>
  <button class="btn-dashboard scale-[0.9] absolute left-[40%] hover_tamkin z-[40] w-1/6 bottom-4 mx-auto" v-if="!showAllFeatures"
   @click="showAllFeatures = true">{{$t('Compare features')}}</button>
 
  <div class="flex items-center justify-center flex-col w-full relative" >
   
   
    <div
      class="flex items-center justify-between ipad-max:flex-col ipadmax:justify-center mt-[32px] w-full  top-[70px] z-[20]"
      :class="[
        packagesStore.currentType.title === 'Sign language' ? 'bg-[#F4F8FF]' : 'bg-white',
        showAllFeatures ? 'sticky':''
      ]"
    >
      <div class="rtl:text-right ltr:text-left mb-8 w-2/4 ipad-max:w-full">
        <h2 class="text-lg font-semibold text-black">{{$t('Compare features')}}</h2>
      </div>

      <div class="flex justify-end divide-x-2 rtl:divide-x-reverse divide-white w-full" >
        <!-- Free -->
        <div class="bg-tamkinEnd/30  rtl:rounded-r-xl ltr:rounded-l-xl p-6 w-1/4 ipad-max:w-full text-center">
          <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse w-full">
            <h3 class="lg:text-[12px] xl:text-[14px] text-center font-semibold text-black">
              {{
                packagesStore.getPackageByTypeAndCategory("Package").length &&
                packagesStore.getPackageByTypeAndCategory("Package")[0]
                  ? $t(packagesStore.getPackageByTypeAndCategory("Package")[0].title)
                  : ""
              }}
            </h3>
            <div
          ...
          >
            <span
              v-if="packagesStore.getPackageByTypeAndCategory('Package')[0].is_contact_us"
            >
              {{ $t('Contact us') }}
            </span>
            <span
        ...
        <div class="bg-tamkinEnd/30 p-6 w-1/4 ipad-max:w-full text-center">
            ...
            <h3 class="lg:text-[12px] xl:text-[14px] text-center font-semibold text-black">
              {{
                ...
                packagesStore.getPackageByTypeAndCategory("Package")[1]
                  ? $t(packagesStore.getPackageByTypeAndCategory("Package")[1].title)
                  : ""
              }}
            </h3>
            <div
          ...
          >
            <span
              v-if="packagesStore.getPackageByTypeAndCategory('Package')[1].is_contact_us"
            >
              {{ $t('Contact us') }}
            </span>
            <span
        ...
        <!-- Teams -->
        <div class="bg-tamkinEnd/30 rtl:rounded-l-xl ltr:rounded-r-xl p-6 w-1/4 ipad-max:w-full text-center">
          <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
            <h3 class="lg:text-[12px] xl:text-[14px] text-center font-semibold text-black">
              {{
                packagesStore.getPackageByTypeAndCategory("Package").length &&
                packagesStore.getPackageByTypeAndCategory("Package")[2]
                  ? $t(packagesStore.getPackageByTypeAndCategory("Package")[2].title)
                  : ""
              }}
            </h3>
            <div
          ...
          >
            <span
              v-if="packagesStore.getPackageByTypeAndCategory('Package')[2].is_contact_us"
            >
              {{ $t('Contact us') }}
            </span>
            <span
              ...
              {{
                packagesStore.getPackageByTypeAndCategory("Package")[2]
                  ...
                  ? `${$t('Try now for')} ${
                      ...
                      packagesStore.getPackageByTypeAndCategory("Package")[2].trial_days
                    ))}`
                  : ""
              }}
            </span>
    ...
    <div v-for="(section, name, index) in filteredSections" :key="index" class="w-full" >
      <div v-if="!showAllFeatures && name === 'addons'">
        ...
        class="bg-[#ECECF0] cursor-pointer w-full h-[60px] mt-[16px] flex items-center justify-between px-[20px]"
        :class="[openedTabs.includes(name) ? 'rounded-t-[10px]' : 'rounded-[10px]']"
      >
      ...
      <div class="w-full h-full" v-if="openedTabs.includes(name)">
        <table
          class="min-w-full min-h-full overflow-visible table-fixed border-collapse border bg-white border-gray-200 relative"
        >
          <tbody>
            <!-- First Row -->
      ...
      <div class="w-full h-full" v-if="openedTabs.includes(name) && showAllFeatures">
        <table
          class="min-w-full min-h-full overflow-visible table-fixed border-collapse border bg-white border-gray-200 relative"
        >
          <tbody>
  ...
  <button class="btn-dashboard  left-[40%] hover_tamkin mt-[16px] z-[40] w-1/6 bottom-4 mx-auto scale-[0.9]" v-if="showAllFeatures" @click="showAllFeatures = false">{{$t('Close features')}}</button>
