...
<template>
  ...
  >
    <div class="market_card_char !justify-center order-1"  @click="openModalAndHideChat(), setData(null)">
      <div>
        <img
      ...
      <!-- <div v-if="char.specialOffer || char.offer_cost > 0 || char.package" class="flex flex-col"> -->
        ...
        <div v-if="marketStore.cartable(char) && char.offer_cost > 0"
          ...
          <div class="flex items-start flex-col justify-evenly space-y-[10px] ">
            ...
            >
              <div>%{{ (((char.cost - char.offer_cost) / char.cost) * 100).toFixed(2) }} {{$t('OFF')}}</div>
            </div>
          ...
          <div
            ...
            class="cursor-pointer group w-[35px] mt-[4px] h-[35px] rtl:mr-auto ltr:ml-auto hover:border-0 bg-white dark:bg-tamkinDarkPrimary hover:bg-gradient-to-b from-tamkinStart to-tamkinEnd rounded-lg flex items-center justify-center dark:border-darkborder border"
          >
            <svg
              :class="[
                marketStore.isInCart(char.name)
...
<div v-if="loadingChars" class="grid grid-cols-1 ipad-max:grid-cols-3 lg:grid-cols-5 md:grid-cols-4 overflow-x-hidden 2xl:grid-cols-5
bg-white dark:bg-[#344153] pt-4 !pb-4 px-[15px] rounded-b-[10px]  gap-4 lg:gap-2 2xl:gap-2 ipad-max:gap-8 relative z-[10]">
