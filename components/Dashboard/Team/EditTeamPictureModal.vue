...
<template>
  <div v-if="isOpen('editteampic')"
    ...
    <div v-bind="getRootProps()" style="
      ...
      <div v-else-if="currTeam.team_image && !isImageDeleted" class="upload-file-item">
      ...
      <div class="w-[264px]">
        <h1 class="text-[13px] leading-[19.5px] font-[400] text-center text-darkGrey dark:text-whiteTamkin">
          Select a high-quality image to represent your team and <span class="font-[500] text-[#2DADA3]">upload it
            here</span>
        </h1>
      </div>
    </div>
    ...
    <div class="flex items-center justify-center space-x-[30px] mx-auto mt-[40px]">
      <button class="flex items-center justify-center space-x-[6px] btn_bordered_dashboard error w-1/4"
        @click="removeFile" :class="!currTeam?.team_image && acceptedFilesRef.length === 0 && 'opacity-40'"
      ...
      <button :disabled="(acceptedFilesRef.length == 0 && !isImageDeleted) || uploadLoading || deleteLoading"
        ...
        <img v-if="uploadLoading || deleteLoading" class="inline-block mx-2" src="/assets/imgs/loading.svg" /> Save
      </button>
    </div>
  </div>
</template>
