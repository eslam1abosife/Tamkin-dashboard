  ...
  <div v-if="isOpen('invitemember')"
    ...
    <div class="container mx-auto max-h-[100%]">
      ...
      <div class="space-y-[44px]">
        <h6 v-if="isIncludeWord(errorMsg, ['something wrong'])"
    ...
    $t("Last name is required")
        ...
        <div class="w-full relative">
          <input @input="clearFieldError(isIncludeWord(errorMsg, ['member', 'agency']))" type="email"
            placeholder="{{$t('email')}}" id="email" class="input_floating_label peer" v-model="v$.email.$model" :class="{
    input_error:
      (v$.email.$error && v$.email.required.$invalid) ||
      (v$.email.$error && v$.email.email.$invalid) ||
      isIncludeWord(errorMsg, ['member', 'agency']),
    input_success: !v$.email.$error && !v$.email.$invalid && !isIncludeWord(errorMsg, ['member', 'agency']),
  }" />
          <label for="email" class="floating_label" :class="[
    (v$.email.$error && v$.email.required.$invalid) ||
      (v$.email.$error && v$.email.email.$invalid) || isIncludeWord(errorMsg, ['member', 'agency'])
      ? '!text-error'
      : '',
  ]">
            {{ $t("email") }}*
    ...
    errorMsg
      ...
      <div class="mt-[32px] w-2/6 mx-auto">
        ...
        <button :disabled="v$.email.$invalid || v$.firstName.$invalid || v$.lastName.$invalid || loadinginvite"
          ...
          <div class="flex items-center justify-center">
            <div :class="loadinginvite ? 'mr-2':''">
             Invite Member
            </div>
       
             <svg  v-if="loadinginvite" class="animate-spin  h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
