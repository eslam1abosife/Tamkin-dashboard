...
<template>
  <div
    ref="navContainer"
    class="absolute top-[225px] z-[50] flex items-center justify-start space-x-[50px] left-0 w-full mt-[18px]"
  >
    <div
      class="relative border-b-1 border-transparent cursor-pointer"
      style="text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.1)"
      @click="selectTabs('webplugins')"
    >
      <div
        class="text-[14px] leading-[24px]"
        :class="[
          selectedTab === 'webplugins'
            ? 'font-[600] text-black'
            : 'font-[500] text-darkGrey',
        ]"
      >
        Web Plugins
      </div>
      <div
        v-if="selectedTab === 'webplugins'"
        class="absolute inset-0 top-full h-[2px] bg-gradient-to-b from-[#2DADA3] to-[#71DAD2]"
      ></div>
    </div>

    <div
      class="relative border-b-1 border-transparent cursor-pointer"
      @click="selectTabs('Media')"
    >
      <div
        class="text-[14px] leading-[24px]"
        :class="[
