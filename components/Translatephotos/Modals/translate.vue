...
>
  <div style="box-shadow: 1px 0px 20.5px 0px #71dad2bd" class="close_btn " @click="closeModal('translate_images')">
    <svg
      class="w-[12px] h-[12px]"
      width="14"
      height="13"
       ...
       justify-center flex-col space-y-[10px] ">
        ...
        space-y-[10px] md:space-y-0 space-x-[16px]" v-if="acceptedFilesRef.length > 0">
          <div v-for="file in acceptedFilesRef" :key="file.name" class="rounded-[10px] w-full md:w-auto">
            <div class="flex items-center justify-start w-full space-x-[14px]">
              <div class="relative " >
                <div class="h-[91px] w-[60px] absolute inset-y-0 right-0 backdrop-blur-sm rounded-tr-[7px] rounded-br-[7px] bg-opacity-40" :style="{ width: blurWidth + '%' }"> </div>
                <div  class="flex items-center justify-center ">
                  <img :src="imgUrl" class="w-auto h-[91px] rounded-[7px]" @click.stop />
                 </div>
              
               
              </div>
              <div class="flex flex-col items-start justify-between space-y-[8px] mb-[48px]">
          ...
          <div class="w-auto  md:w-auto  ">
            <button class="text-red-500 hover:bg-[#FFF3F2] hover:border-[#FACECB] w-[32px] h-[32px] mb-[60px]
        ...
        <div>
          <h1 class="text-[13px] leading-[19.5px] font-[400] text-center text-[#052443] dark:text-whiteTamkin" v-if="acceptedFilesRef.length === 0">
            <span class="text-tamkin cursor-pointer">Click here</span> to upload or drop images
          </h1>
        </div>
  ...
  <div class="flex flex-col items-center justify-center px-[50px] h-[600px] pb-[16px] space-y-[20px] dark:bg-tamkinDarkPrimary w-full mx-auto rounded-[10px] mt-[16px]" style="box-shadow: 0px 4px 24px 8px #51459f14" v-else-if="rendering && !failedRender">
    ...
    <div class="text-[24px] leading-[30px] font-[600] text-darkGrey">
     <span class="capitalize">Image </span> is processing  
    </div>
