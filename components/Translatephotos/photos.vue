...
<template>
   <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 ipad-max:grid-cols-3 mt-[10px] ">
        ...
        <div v-for="item in 10" :key="item" class="h-[140px]  flex flex-col items-start justify-start rounded-[7px] shadow-lg group hover:border-tamkin
         hover:border-[1px] cursor-pointer mx-1 mb-2">
        <div class="relative w-full ">
            <div @click="changeDropMenu(item)"
            :class="[currentMenuDrop !== item ? '' : '!flex']"
         ...
         class="text-darkGrey dark:text-whiteTamkin"
            ...
            space-y-[3px]  justify-center" v-if="currentMenuDrop === item">
