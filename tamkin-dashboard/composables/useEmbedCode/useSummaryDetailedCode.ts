...
export default function () {
    ...
    const getSummaryDetailedCode = async () => {
        ...
        try {
            const res = await api.get('/mySite/GetEmbedCode');

            if (!res.data.succeeded) throw(res.data.message);
            const urlSummaryDecodedString = decodeURIComponent(res.data.data.summary_code);
            summaryCode.value = cleanUpCode(decodeHtmlEntities(urlSummaryDecodedString));
            const urlDecodedDecodedString = decodeURIComponent(res.data.data.detiles_code);
