    ...
    <div class="flex items-start justify-start w-full">
      <div class="flex items-center justify-start gap-4 w-full">
        <div class="relative">
          ...
          <div
            v-if="
              !profileStore.member.user_image &&
              (profileStore.member.first_name ||
                profileStore.member.last_name) &&
              !profileStore.loadingProfile
            "
            class="avatar_img h-[60px] text-[32px] w-[60px] lg:h-[80px] lg:w-[80px] ipad-max:w-[50px] ipad-max:h-[50px] rounded-full bg-[#2dada3] text-white grid place-content-center select-none"
          >
            <span>
              {{
                getAvatarLetters(
        ...
        <!-- Member Details -->
        <div
          class="flex flex-col items-start justify-start space-y-1 w-2/4 truncate"
        >
          <div
            v-if="!profileStore.loadingProfile"
            class="text-[14px] dark:text-whiteTamkin lg:text-[14px] 2xl:text-[16px] ipad-max:text-[12px] truncate ipad-max:w-14 w-32 2xl:w-44 whitespace-nowrap leading-[22px] text-[#3D3D3D] font-[600]"
          >
            {{ memberFullName }}
          </div>
          <div
            v-else
            class="bg-gray-300 dark:bg-p animate-pulse h-[20px] w-[80px] rounded"
          ></div>

          <div
            v-if="!profileStore.loadingProfile"
            class="text-[11px] lg:text-[13px] dark:text-whiteTamkin font-[500] ipad-max:text-[10px] leading-[20px] text-[#878787]"
          >
          ...
          <div
            class="text-[#616161] text-[11px] dark:text-whiteTamkin lg:text-[13px] ipad-max:text-[10px] font-[500] leading-[18px]"
            v-if="!profileStore.loadingProfile"
          >
            {{
              profileStore.member.country
          ...
          <div
            v-else
            class="bg-gray-300 dark:bg-p animate-pulse h-[20px] w-full rounded"
          ></div>
        </div>
      </div>
      ...
      <div class="absolute rtl:left-[15px] ltr:right-[15px]">
        ...
        >
          <div
            class="text-[11px] dark:text-whiteTamkin lg:text-[12px] font-[500] ipad-max:text-[10px]"
          >
            {{ $t("Active") }}
          </div>
        </div>
        ...
        <div
          class="w-[46px] h-[20px] dark:bg-tamkinDarkPrimary lg:w-[59px] lg:h-[27px] ipad-max:w-[46px] ipad-max:h-[20px] rounded-[23px] text-white bg-gray-300 flex items-center justify-center"
          v-else
        ></div>
      </div>
    </div>
