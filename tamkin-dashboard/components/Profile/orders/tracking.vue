...
<template>
  <div
    v-if="isOpen('tracking_custom_order') && dataModal"
    class="fixed z-[9999] ipad-max:top-[20px] top-[30px] bg-white dark:bg-tamkinDarkPrimary rounded-[10px] p-[30px] h-auto w-[680px]"
  ...
  >
    ...
    <div
      ...
      class="close_btn"
      @click="closeModal('tracking_custom_order')"
    >
      <svg
        class="w-[12px] h-[12px]"
        width="14"
    ...
    <div class="mx-auto max-h-[100%] w-full">
      ...
      <div
        class="flex items-center justify-between w-full mt-[10px] rtl:space-x-reverse space-x-[8px]"
      >
        <div>
          <span
            class="text-[16px] dark:text-whiteTamkin font-[600] leading-[24px] text-darkGrey rtl:ml-2 ltr:mr-2"
          >
            {{ $t("Order ID") }} :</span
          >
          <span
            class="text-[16px] dark:text-whiteTamkin font-[500] leading-[24px] text-[#80889C]"
            >{{ dataModal.id || "" }}</span
          >
        </div>
        ...
        <div class="flex flex-col items-start justify-start space-y-[8px]">
          <div
            class="text-[13px] dark:text-whiteTamkin font-[500] leading-[19px]"
            :class="[
              !dataModal.trakin.some((item) => item.level !== 'Reject')
                ? 'text-darkGrey'
                : 'text-darkGrey/60',
            ]"
          >
          ...
          >
            ...
            <span
              class="text-[12px] leading-[18px] font-[500] dark:text-whiteTamkin"
            >
              {{
                !dataModal.delivary_date
                  ? $t("Date to be Determined")
                  : dataModal.delivary_date
              }}
            </span>
          </div>
      ...
      <div class="w-full mt-[32px]">
        ...
        <div class="relative w-full">
          <!-- Vertical line -->
          <div
            class="absolute top-0 rtl:inset-x-[110px] ltr:inset-x-[110px] h-full w-[12px] rounded-t-full rounded-b-full bg-teal-500"
          ...
          <template v-for="index in 5">
            <div class="relative flex items-start gap-[10.5px] mb-8">
              ...
              >
                <p
                  class="text-[12px] font-[500] text-[#1C1C1C] leading-[18px] dark:text-whiteTamkin whitespace-nowrap"
                  v-html="DateSplit(dataModal.trakin[index - 1].date) || ''"
                  :class="[
                    dataModal.status === 'Reject' ? '!text-[#F02F1F]' : '',
                  ]"
                ></p>
              </div>
              <div class="flex-shrink-0" v-if="dataModal.trakin[index - 1]">
                <div
                  :class="[
                    dataModal.status === 'Reject' ? '!border-[#DADADA]' : '',
                  ]"
                  class="w-8 h-8 bg-white border-[1px] border-[#71DAD2] text-white rounded-full flex items-center justify-center"
                >
                  <svg
                    width="22"
                    height="22"
                  ...
                  >
                    <circle
                      :class="[
                        dataModal.status === 'Reject'
                          ? 'fill-[#DADADA]'
                          : 'fill-[url(#paint0_linear_8989_149244)]',
                      ]"
                      cx="11"
                      cy="11"
                      r="11"
                    ...
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
                      d="M16.7594 7.00028L6.48047 15.125L7.72068 16.694L17.9996 8.56931L16.7594 7.00028ZM5.56904 10.7461L4 11.9863L6.48042 15.1244L8.04946 13.8842L5.56904 10.7461Z"
                      fill="#FFFEFE"
                    />
                    <defs>
                      <linearGradient
                        id="paint0_linear_8989_149244"
                        x1="11"
                        y1="0"
                        x2="11"
                        y2="22"
              ...
              <div v-else class="flex-shrink-0 rtl:pr-[100px] ltr:pl-[100px]">
                <div
                  class="w-8 h-8 bg-white border-[1px] border-[#F2F2F2] text-white rounded-full flex items-center justify-center"
                >
                  <div
                    class="w-[22px] h-[22px] rounded-full bg-[#DAF3F1]"
                    :class="[
                      dataModal.status === 'Reject' ? '!bg-[#DADADA]' : '',
                    ]"
                  ></div>
                </div>
              </div>
              ...
              <div class="rtl:mr-6 ltr:ml-6">
                <h4
                  class="text-[16px] leading-[27px] font-[600] dark:text-whiteTamkin text-darkGrey"
                  :class="[
                    dataModal.status === 'Reject' ? '!text-[#F02F1F]' : '',
                  ]"
                >
                ...
                <p
                  class="text-[13px] text-[#80889C] leading-[19px] font-[500] w-[400px]"
                  v-if="dataModal.trakin[index - 1]"
                  :class="[
                    dataModal.status === 'Reject' ? '!text-[#F49E97]' : '',
                  ]"
                >
        ...
        <div
          class="h-[60px] w-full rounded-[10px] bg-[#FFE9E9] flex items-center px-[20px] rtl:space-x-reverse space-x-[12px] justify-start"
        ...
        >
          <div class="ltr:self-start rtl:self-center ltr:mt-[12px]">
            ...
            >
              <path
                d="M7.49984 15C11.5957 15 15 11.6029 15 7.5C15 3.40427 11.5881 0 7.49231 0C3.3896 0 0 3.40427 0 7.5C0 11.6029 3.39681 15 7.49984 15ZM7.50016 13.7501C4.02899 13.7501 1.25651 10.9705 1.25651 7.5C1.25651 4.03643 4.02146 1.2499 7.49231 1.2499C10.9556 1.2499 13.7422 4.03675 13.7498 7.5C13.757 10.9708 10.9629 13.7501 7.49953 13.7501M7.49231 8.83084C7.84495 8.83084 8.04355 8.63225 8.05076 8.25013L8.1612 4.36773C8.16873 3.99251 7.87444 3.71329 7.48478 3.71329C7.0876 3.71329 6.80837 3.9853 6.81559 4.3602L6.91128 8.25013C6.91849 8.62503 7.12462 8.83084 7.49231 8.83084ZM7.49231 11.2205C7.91115 11.2205 8.28607 10.8823 8.28607 10.456C8.28607 10.0221 7.91868 9.69108 7.49231 9.69108C7.05842 9.69108 6.69794 10.0293 6.69794 10.456C6.69794 10.8751 7.06563 11.2205 7.49231 11.2205Z"
                fill="#585B5B"
              />
            </svg>
          </div>
          <div class="text-[13px] font-[500] leading-[19.5px] text-[#585B5B]">
            {{
              $t(
                dataModal.trakin.find((item) => item.level === "Reject")
                  .description
              )
            }}
          </div>
