...
watch(
  ...
  (newState) => {
    ...
    profileStore.updateProfilePayload = {
      ...state,
      phone: `00${countryCode.value}${localPhone.value}`,
    };
  },
  { deep: true }
);
...
<template>
  <div class="w-full">
      ...
      <div class="w-full relative">
        ...
        >
          <p class="error_message">
            <span
              v-if="v$.first_name.$error && v$.first_name.required.$invalid"
              >{{ $t("Please enter The First Name") }}</span
            >
          </p>
        </div>
      </div>
      ...
      <div class="w-full relative">
        ...
        >
          <p class="error_message">
            <span
              v-if="v$.last_name.$error && v$.last_name.required.$invalid"
              >{{ $t("Please enter The Last Name") }}</span
            >
          </p>
        </div>
      </div>
    ...
    >
      <button class="btn_bordered_dashboard" @click="cancelUpdate">
        {{ $t("Cancel") }}
      </button>
      <button
        class="btn-dashboard hover_tamkin w-[125px]"
        :disabled="loadingPersonal || v$.$invalid"
...
<style>
.vue-tel-input {
  @apply rounded-[10px] bg-white h-[40px] dark:bg-transparent;
}
...
.vti__dropdown-list.below {
  @apply ipad-max:!w-[400px] lg:!w-[250px] 2xl:!w-[340px] rounded-[10px] dark:bg-transparent bg-white border border-[#D9D9D9] top-[52px];
}
.vti__dropdown {
  @apply rounded-[10px] bg-white dark:bg-transparent;
}
.vti__dropdown-item {
  @apply px-[16px] py-2 text-[12px] dark:text-whiteTamkin !font-[Poppins] !font-[300];
}

.vti__dropdown-item.highlighted {
  @apply bg-tamkinLight dark:text-whiteTamkin;
}
