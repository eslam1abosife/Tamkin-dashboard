...
loadingPacks.value = false
  ...
  if (mySiteStore.currentWebsite && Array.isArray(mySiteStore.currentWebsite.package)) {
    mySiteStore.currentPackage = mySiteStore.currentWebsite.package.find(pck => pck.type === 'Sign language');
  } else {
...
const getCurrentAccessPackage = computed(() => {
  return navStore.defaultappobj && Array.isArray(navStore.defaultappobj.package) && navStore.defaultappobj.package.length 
...
>
    ...
    >
      ...
      >
        <div class="flex flex-col items-start justify-center w-full px-[15px]">
          ...
          <div v-if="navStore.defaultappobj?.package?.length && navStore.defaultappobj.package.find(p => p.type === 'Sign language')"
          ...
          >
            <div class=" flex items-center justify-start rtl:space-x-reverse space-x-[8px] w-full ">
              <div>
             <img  
             :src="runtimeConfig.public.baseImagerUrl + (getCurrentAccessPackage ? getCurrentAccessPackage.icon : '/')" 
  ...
  ipad-max:leading-[10px] whitespace-nowrap
    ...
    {{$t(getCurrentAccessPackage.title)}} - {{$t(`${getCurrentAccessPackage.type}` ) }}  
        ...
        >
          ...
          <div
          v-if="getCurrentAccessPackage.status  === 'Rejected' || getCurrentAccessPackage.status === 'Rejected' || getCurrentAccessPackage.status === 'Cancelled'"
        ...
        >
          ...
          >
            {{ getCurrentAccessPackage.status === 'Pending'? $t('Under Review') : $t(`${getCurrentAccessPackage.status}`) }}
          </div>
