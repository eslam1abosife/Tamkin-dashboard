...
watchEffect(() => {
  if (
    ...
    statsStore.chartsData?.opencount?.length > 0 &&
    statsStore.chartsData?.loadscount?.length > 0
  ) {
...
<template>
  <div
    v-if="!loadingStts"
    class="flex flex-col items-center justify-center w-full my-[30px]"
  >
    <div
      class="bg-white scrollbar-thin dark:bg-tamkinDarkPrimary overflow-x-auto rounded-[10px] w-full shadow-md relative px-[15px]"
      :class="[
        collapseStore.collapses.includes('webplugins_chart_card')
          ? 'pb-[24px]'
          : 'pb-[10px]',
      ]"
    >
      ...
      <div
        v-if="!collapseStore.collapses.includes('webplugins_chart_card')"
        class="container_chart mt-[30px] h-[300px] w-[700px] sm:w-[800px] lg:w-full overflow-x-auto relative"
      >
        <Line
          ref="chart12"
          :data="chartData"
    ...
    >
      ...
      >
        ...
        <div class="space-y-2">
          ...
          ></div>
          <div
            class="h-3 bg-gray-200 dark:bg-tamkinDarkPrimary rounded w-1/4"
          ></div>
        </div>
      </div>
      ...
      >
        <div
          class="bg-gray-300 animate-pulse dark:bg-p h-full w-full rounded-md"
        ></div>
      </div>
    </div>
  </div>
