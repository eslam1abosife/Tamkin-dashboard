...
<template>
  <div
    class="flex flex-col items-center justify-center w-full mt-[40px] relative mb-[80px]"
  >
    <div
      class="bg-white dark:bg-tamkinDarkPrimary rounded-[10px] w-full px-[15px] relative shadow-md -shadow-y-[1px]"
      :class="[
    ...
    >
      <div class="flex items-center justify-start pt-[16px] relative">
        <div class="flex flex-col items-start justify-center relative">
          <div>
            ...
            >
              Custom Trigger enables users to define specific conditions or
              events that automatically initiate tailored actions
            </p>
          </div>
          ...
          >
            ...
            >
              A custom trigger replaces the original menu button and allows you
              to fix the position of the accessibility menu trigger to any
              element on your website. Open the accessibility menu when clicking
              an element with a pre-defined ID or class. To specify a class
              instead of an ID, precede the value with a period (.)
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        class="flex flex-col items-start justify-center mt-[18px] max-w-full"
        :class="[!moveAccess ? 'pb-[16px]' : '']"
        v-if="!collapseStore.collapses.includes('custom_trigger_card')"
      >
        <div
        ...
        >
          <div class="flex items-center justify-start space-x-[13px] w-full">
            ...
            <div class="ltr:ml-auto rtl:mr-auto">
              <label for="custom_trigger_id" class="toggle_wrap">
                <input
                  type="checkbox"
                  id="custom_trigger_id"
                  class="sr-only"
                  :checked="isChecked('custom_trigger_id')"
                  @change="toggleCheckbox('custom_trigger_id')"
                />
                <div
                  class="toggle_parent"
                  :class="[
                    isChecked('custom_trigger_id') ? 'active' : 'in_active',
                  ]"
                >
                  <div
                    class="toggle_inner"
                    :class="{ active: isChecked('custom_trigger_id') }"
                  >
                    <img
        ...
        >
          ...
          <div
            ...
            v-if="v$.custom_id.$error && v$.custom_id.required.$invalid"
          >
            <p class="error_message">
              <span
                v-if="v$.custom_id.$error && v$.custom_id.required.$invalid"
                >{{ $t("An Id Is Required") }}</span
              >
            </p>
          </div>
        </div>
        <div
          class="   "
          :class="[!state.custom_id ? 'w-full ' : 'max-w-2xl']"
          v-if="isChecked('custom_trigger_id')"
        >
          <Client-only>
            <VCodeBlock
            ...
            />
            <p
              class="font-[400] text-[#979897] dark:text-whiteTamkin text-[11px] leading-[19px]"
            >
              This ID can be added to any HTML element such as &lt;a&gt;,
              &lt;img&gt;, &lt;div&gt;&lt;/div&gt;, &lt;span&gt;,
              &lt;li&gt;&lt;/li&gt;, etc.
            </p>
          </Client-only>
        ...
        >
          <div
            class="bg-white dark:bg-tamkinDarkPrimary rounded-md flex items-center justify-center"
          >
            <div class="pl-[16px]">
              <svg
                width="20"
