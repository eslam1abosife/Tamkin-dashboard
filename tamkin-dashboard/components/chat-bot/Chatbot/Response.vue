<template>
  ...
  >
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-semibold text-gray-800 dark:text-whiteTamkin">Chatbot Response</h2>
      <button
        @click="$emit('close')"
        style="
          position: absolute;
        ...
        "
        class="text-gray-500 dark:bg-tamkinDarkPrimary hover:text-gray-700 dark:text-gray-300 dark:hover:text-whiteTamkin text-xl font-bold"
      >
        <svg
          width="14"
          height="13"
    ...
    <div class="space-y-4">
      ...
      >
        <input
          ...
          type="text"
          placeholder="Title"
          class="w-full p-2 bg-transparent border-none focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 rounded-lg text-gray-800 dark:text-whiteTamkin placeholder-gray-500 dark:placeholder-gray-300"
        />
      </div>
      <label class="flex items-center space-x-2 p-2 bg-white dark:bg-tamkinDarkPrimary rounded-lg">
        <input
          v-model="createGroup"
          type="checkbox"
          class="form-checkbox text-teal-500 border-gray-300 dark:border-gray-600 rounded focus:ring-teal-500"
        />
        <span class="text-gray-700 dark:text-whiteTamkin">Create Group</span>
      </label>
      <button
        @click="$emit('create-new', title)"
