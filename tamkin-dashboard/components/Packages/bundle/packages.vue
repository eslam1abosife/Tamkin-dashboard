      ...
      <SplideSlide v-for="pak in packagesStore
        ...
        <div
          ...
          <div class="flex justify-center items-center mb-[14px] rtl:space-x-reverse space-x-[12px]">
            ...
            <div>
              <h3 class="text-xl font-semibold dark:text-whiteTamkin text-center text-gray-800">
                {{ $t(pak.title) }}
              </h3>
            </div>
          </div>

          <p class="text-[13px] leading-[19px] dark:text-whiteTamkin font-[400] text-darkGrey text-center">
            {{ $t(pak.sub_title) }}
          </p>
          <!--           
          ...
          <h3 class="text-2xl font-semibold dark:text-whiteTamkin text-center text-gray-800 my-[14px]">
            {{ $t("Bundle") }}
          </h3>

          <div class="w-full">
            <button @click="openModalCustom(pak)" :disabled="pak.cost_month !== 0 &&
              pak.cost_yearly !== 0 &&
              !pak.is_contact_us &&
              pak.trial_days === 0
