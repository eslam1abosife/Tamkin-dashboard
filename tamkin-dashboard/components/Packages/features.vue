...
<template>
  ...
  <div class="w-full relative mt-[23px]" v-if="compareg">
    <div v-if="!showAllFeatures"
      class="bg-gradient-to-t pointer-events-none dark:from-darkTamkin absolute inset-auto h-full w-full z-[40]" :class="[
        currentPage === 'sign'
          ? 'from-[#EEF5FF] via-[#EEF5FF]/30 '
          : 'from-white via-white/30 ',
      ]"></div>
    <button class="btn-dashboard scale-[0.9] absolute left-[50%] -translate-x-1/2 text-nowrap hover_tamkin z-[40] w-fit max-w-full px-6 bottom-4 mx-auto"
      v-if="!showAllFeatures" @click="showAllFeatures = true">
      {{ $t("Compare features") }}
    </button>

    <div class="flex items-center dark:text-whiteTamkin justify-center flex-col w-full relative">
      <div
        class="hidden xl:flex items-center p-2 ps-6 justify-between dark:bg-tamkinDarkPrimary border-[1px] dark:border-darkborder dark:text-whiteTamkin mt-[32px] w-full top-[70px] z-[20]"
        :class="[
        ...
        <div class="rtl:text-right ltr:text-left text-center w-2/4">
          <h2 class="text-lg font-semibold text-black dark:text-whiteTamkin mb-0">
            {{ $t("Compare features") }}
          </h2>
        </div>

        <div
          class="flex justify-center lg:justify-end divide-x-[1px] xl:divide-x-2 rtl:divide-x-reverse divide-white w-2/4">
          <!-- Free -->
          <div class="bg-tamkinEnd/30 rtl:rounded-r-xl ltr:rounded-l-xl p-4 xl:p-6 min-w-[123px] flex-1  text-center">
            <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse w-full">
              <h3 class="text-[12px] xl:text-[14px] text-center dark:text-whiteTamkin font-semibold text-black">
                {{
            ...
            <button v-if="packagesStore.getPackageByTypeAndCategory('Package')[0]" @click="
              openBuyModal(
                packagesStore.getPackageByTypeAndCategory('Package')[0],
                packagesStore.getPackageByTypeAndCategory('Package')[0]
                  .is_contact_us
              )
              " class="mt-4 xl:mt-6 btn_bordered_dashboard !text-[12px] md:!text-[14px] !p-1 w-full hover_tamkin text-nowrap">
              <span v-if="
                packagesStore.getPackageByTypeAndCategory('Package')[0]
                  .is_contact_us
              ">
          ...
          <!-- Pro -->
          <div class="bg-tamkinEnd/30 p-4 xl:p-6 min-w-[123px] flex-1 text-center">
            ...
            <button v-if="packagesStore.getPackageByTypeAndCategory('Package')[1]" @click="
              openBuyModal(
                packagesStore.getPackageByTypeAndCategory('Package')[1],
                packagesStore.getPackageByTypeAndCategory('Package')[1]
                  .is_contact_us
          ...
          <!-- Teams -->
          <div class="bg-tamkinEnd/30 rtl:rounded-l-xl ltr:rounded-r-xl p-4 xl:p-6 min-w-[123px] flex-1 text-center">
            <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
              <h3 class="text-[12px] xl:text-[14px] dark:text-whiteTamkin text-center font-semibold text-black">
                {{
                  packagesStore.getPackageByTypeAndCategory("Package").length &&
            ...
            <button v-if="packagesStore.getPackageByTypeAndCategory('Package')[2]" @click="
              openBuyModal(
                packagesStore.getPackageByTypeAndCategory('Package')[2],
                packagesStore.getPackageByTypeAndCategory('Package')[2]
                  .is_contact_us
              ...
              ">
                {{
                  packagesStore.getPackageByTypeAndCategory("Package")[2]
                    .package_price_role[0].trial_days > 0
                    ? `${$t("Try now for")} ${packagesStore.getPackageByTypeAndCategory("Package")[2]
                      .package_price_role[0].trial_days
                    } ${$t(
                      getDayLabel(
      ...
      <div v-for="(section, name, index) in filteredSections" :key="index" class="w-full">
        <div v-if="!showAllFeatures && name === 'addons'">
          <div @click="toggleTab(name)"
            ...
            <div class="flex items-center justify-center gap-4">
              ...
              <svg v-if="name === 'support'" class="size-6" data-slot="icon" fill="none" stroke-width="1.5"
              ...
              <svg v-if="name === 'management'" class="size-6" data-slot="icon" fill="none" stroke-width="1.5"
                stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 0 0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5A3.375 3.375 0 0 0 6.375 7.5H5.25m11.9-3.664A2.251 2.251 0 0 0 15 2.25h-1.5a2.251 2.251 0 0 0-2.15 1.586m5.8 0c.065.21.1.433.1.664v.75h-6V4.5c0-.231.035-.454.1-.664M6.75 7.5H4.875c-.621 0-1.125.504-1.125 1.125v12c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V16.5a9 9 0 0 0-9-9Z">
                </path>
              </svg>
              <div class="text-[17px] !font-[700] text-[#18181B] dark:text-whiteTamkin leading-[20px]">
                {{ $t(name.toUpperCase()) }}
              </div>
            </div>

            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-6" :class="[!openedTabs.includes(name) ? 'rotate-180' : '']">
              <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
            </svg>
          </div>
          <div class="w-full h-full overflow-y-auto dark:bg-tamkinDarkPrimary dark:text-whiteTamkin" v-if="openedTabs.includes(name)">
            <table
              ...
              <tbody class="dark:bg-tamkinDarkPrimary dark:text-whiteTamkin">
                ...
                <tr class="contents xl:hidden dark:bg-tamkinDarkPrimary dark:text-whiteTamkin">
                  <td class="px-6 w-2/4 py-4 border border-gray-200">
                    <h2 class="text-lg font-semibold text-black dark:text-whiteTamkin mb-0 whitespace-nowrap">
                      {{ $t("Compare features") }}
                    </h2>
                  </td>
                  <td class="w-1/6 p-4 xl:px-6 border border-gray-200 bg-tamkinEnd/30 text-center min-w-[123px]">
                      <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse w-full">
                        <h3
                          class="text-[12px] xl:text-[14px] text-center dark:text-whiteTamkin font-semibold text-black">
                          {{
                            packagesStore.getPackageByTypeAndCategory("Package").length &&
                              packagesStore.getPackageByTypeAndCategory("Package")[0]
                              ? $t(
                  ...
                  <td class="w-1/6 p-4 xl:px-6 border border-gray-200 bg-tamkinEnd/30 text-center min-w-[123px]">
                      <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse">
                        <h3
                          class="text-[12px] xl:text-[14px] dark:text-whiteTamkin text-center font-semibold text-black">
                          {{
                            packagesStore.getPackageByTypeAndCategory("Package").length &&
                              packagesStore.getPackageByTypeAndCategory("Package")[2]
                              ? $t(
                ...
                <tr class="dark:bg-tamkinDarkPrimary dark:text-whiteTamkin" v-for="(value, key, i) in currentPage === 'sign' &&
                  showAllFeatures
                  ? uniqueValues(
                  ...
                  <td class="px-6 w-2/4 py-4 text-sm text-gray-700 border border-gray-200">
                    <div
                      ...
                      <span v-if="value && value.note" class="tooltip packages" :data-tamkin="$t(value.note)">
                        <svg width="16" class="ml-auto" height="15" viewBox="0 0 16 15" fill="none"
                  ...
                  <td class="px-6 w-1/6 py-4 text-sm text-center text-gray-700 border border-gray-200"
                    v-if="filteredItem(name, 0, value.title)">
                    <div class="flex justify-center items-center h-full" v-if="
                      filteredItem(name, 0, value.title).is_available &&
                      !filteredItem(name, 0, value.title).description
                    ">
                      <img :src="filteredItem(name, 0, value.title).is_available
                        ? '/assets/imgs/checked_list_active.svg'
                        : '/assets/imgs/checked_list_inactive.svg'
                        " />
                    </div>
                    <div v-else-if="filteredItem(name, 0, value.title).description" class="text-[14px]"
                  ...
                  <td class="px-6 w-1/6 py-4 text-sm text-center text-gray-700 border border-gray-200"
                    ...
                    ">
                      <img :src="filteredItem(name, 1, value.title).is_available
                        ? '/assets/imgs/checked_list_active.svg'
                        : '/assets/imgs/checked_list_inactive.svg'
                        " />
                    </div>
                    <div v-else-if="filteredItem(name, 1, value.title).description" class="text-[14px]"
                      style="color: rgb(53, 215, 215)">
                  ...
                  <td class="px-6 w-1/6 py-4 text-sm text-center text-gray-700 border border-gray-200"
                    ...
                    ">
                      <img :src="filteredItem(name, 2, value.title).is_available
                        ? '/assets/imgs/checked_list_active.svg'
                        : '/assets/imgs/checked_list_inactive.svg'
                        " />
                    </div>
                    <div v-else-if="filteredItem(name, 2, value.title).description" class="text-[14px]"
                      style="color: rgb(53, 215, 215)">
        ...
        <div v-if="showAllFeatures" @click="toggleTab(name)"
          ...
          <div class="flex items-center justify-center gap-4">
            ...
            <div class="text-[17px] dark:text-whiteTamkin !font-[700] text-[#18181B] leading-[20px]">
              {{ $t(name.toUpperCase()) }}
            </div>
          </div>
        ...
        <div class="w-full h-full overflow-x-auto dark:bg-tamkinDarkPrimary dark:text-whiteTamkin"
        v-show="openedTabs.includes(name) && showAllFeatures">
          <table
    ...
    <button class="btn-dashboard text-nowrap hover_tamkin mt-[16px] z-[40] w-fit max-w-full px-6  mx-auto scale-[0.9]"
      v-if="showAllFeatures" @click="showAllFeatures = false">
      {{ $t("Close features") }}
    </button>
  </div>
</template>
