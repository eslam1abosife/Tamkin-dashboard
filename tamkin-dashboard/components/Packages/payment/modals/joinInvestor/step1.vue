...
const rules = {
  ...
  website_new: {
    ...
    isDomain: helpers.withParams({ type: "isDomain" }, (value) => {
...
const selectedpcks = ref(
  mySiteStore.currentWebsite?.package?.package_name || ""
);
...
const filteredCryptoMethods = computed(() => {
  return cryptoStore.list.find((method) => method.title === "TSLT");
});
/**
...
<template>
  ...
  <div
    ...
    class="close_btn !static lg:!absolute"
    @click="
      () => {
        closeModal('join_to_investor');
        currentWebSite = '';
  ...
  >
    ...
    >
      ...
      <div
        class="flex items-center justify-center rtl:space-x-reverse space-x-[18px]"
      >
        <div>
      ...
      >
        {{
          $t(
            "Based on the detailed information you’ve provided, a personalized package will be carefully determined to meet your specific needs and preferences"
          )
        }}
      </div>
    </div>
    ...
    <div class="w-full overflow-x-hidden overflow-y-auto max-h-[500px]">
      ...
      <div
        class="mx-auto text-center text-[14px] mt-[7px]"
        v-if="!mySiteStore.currentWebsite"
      >
        {{ $t("OR") }}
      </div>
      <div
        class="flex items-center justify-center w-full mt-[7px] gap-4"
        v-if="!mySiteStore.currentWebsite"
      >
        <div class="w-full relative">
          <input
            ...
            :class="{
              ...
              error_text:
                ...
                (v$.website_new.$error && v$.website_new.isDomain.$invalid),
              input_success:
                !v$.website_new.$invalid && !blockedError && !websiteExists,
            }"
          />
          ...
          <div
            class="w-full"
            v-if="
              (v$.website_new.$error && v$.walletAddress.required.$invalid) ||
              blockedError ||
              websiteExists ||
              (v$.website_new.$error && v$.website_new.isDomain.$invalid)
            "
          >
            <p class="error_message !bottom-[1px]">
              <span
                v-if="
                  v$.website_new.required.$invalid ||
                  (v$.website_new.$error && v$.website_new.isDomain.$invalid)
                "
        ...
        <div class="w-[150px]">
          <button
            ...
            :disabled="
              ...
              websiteExists ||
              currentWebSite !== '' ||
              v$.website_new.$invalid
            "
            @click="lockedWebsite = true"
            v-if="!lockedWebsite || currentWebSite || state.website_new === ''"
          >
            {{ $t("Add site") }}
          </button>
          <button
            class="btn_bordered_dashboard error w-[150px]"
            @click="lockedWebsite = false"
            v-if="lockedWebsite && state.website_new"
          >
            {{ $t("Remove site") }}
          </button>
        </div>
      ...
      <div class="w-full mt-[14px]">
        <TranslateSelectInput
          ...
          class=""
          :current-list-value="
            mySiteStore.currentWebsite?.package?.package_title ||
            mySiteStore.currentWebsite?.package?.title ||
            ''
          "
        />
      </div>
        ...
        <input
          ...
          :class="{
            ...
            error_text:
              v$.walletAddress.$error && v$.walletAddress.required.$invalid,
            input_success:
              !v$.walletAddress.$error && !v$.walletAddress.$invalid,
          }"
        />
        <label
          for="email"
          ...
          <p class="error_message">
            <span
              v-if="
                v$.walletAddress.$error && v$.walletAddress.required.$invalid
              "
              >{{ $t("Wallet Address is required") }}</span
            >
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col items-center w-full">
        <div
          v-for="(hashAddress, index) in state.hashAddresses"
          :key="index"
          class="w-full relative mt-[14px]"
        >
          <button
