...
<template>
  <div class="py-[17px] search_input w-full rtl:!mr-[40px] ltr:!ml-[40px] relative">
    <input
      type="text"
    ...
    <div v-if="isSearchfilled" class="bg-white w-full max-h-[400px] h-auto overflow-y-auto absolute rounded-[10px] border-[1px] drop-shadow-xl top-16 flex flex-col items-start justify-start p-4">
      <div v-if="!loadingSearch && searchData.length" class="w-full"> 
        <div v-for="site in searchData" :key="site.id" class="flex items-center divide-y justify-between p-3  rounded-[10px]
         cursor-pointer w-full hover:bg-gray-50"> 
          <div class="flex items-center justify-start space-x-4 rtl:space-x-reverse w-full">
            <div>
              <img
              ...
              />
              <div v-else-if="!site?.favicon && site?.title !== 'Internal Service'"
            ...
            <button @click="()=>{
              $router.push({ path: localePath('/my-site'), query: { ping: site.name } })
      ...
      <div v-if="noresult" class="w-full">
        <div class="text-center text-[14px] font-[500] leading-[24px]">{{$t('No Result')}}</div>
      </div>
