...
const selectCryptoMethod = (method) => {
  if (addSiteStore.selectedCrypto !== method) {
...
onBeforeMount(async () => {
  ...
  if (!addSiteStore.selectedCrypto) {
    addSiteStore.selectedCrypto = featuredRate;
  }
...
const discountAmount = computed(() => {
  const cartTotal = addSiteStore.packagePayload.total;
  const cryptoDiscount = addSiteStore.selectedCrypto?.discount || 0;
  const couponDiscount = addSiteStore.currentDiscount;
    ...
    <div class="w-full h-full">
      <div class="flex flex-col items-start justify-center w-full" v-if="isModalOpen('crypto_addsite_step1')">
        ...
        <div
          ...
          :class="[loadingData ? 'pb-[120px]' : '']" style="box-shadow: 0px 4px 24px 8px #51459f14">
          <h1
          ...
          <div class="flex flex-col items-center justify-center space-y-[12px] mt-[24px] mx-auto w-full"
            v-if="!loadingData">
            <div class="flex flex-col items-center justify-center w-full px-4 md:px-[20px]">
              <div v-if="addSiteStore.selectedCrypto"
                ...
                <div class="flex items-center justify-start rtl:space-x-reverse space-x-[10px]">
                  <img :src="`http://tamkin.app/${addSiteStore.selectedCrypto.icon}`" class="w-[25px] h-[25px]" />
                  <div class="flex items-start flex-col justify-start space-y-[-4px]">
                    <div
                      class="md:text-[18px] text-[14px] leading-[44px] font-[600] font-[Inter] text-darkGrey dark:text-whiteTamkin">
                      {{ addSiteStore.selectedCrypto.title }} -
                      <span class="!font-[300]">{{
                        addSiteStore.selectedCrypto.network
                      }}</span>
                    </div>

                    <div class="text-[#021328] text-[12px] md:text-[14px] font-[500] dark:text-whiteTamkin" v-if="
                      addSiteStore.selectedCrypto &&
                      addSiteStore.selectedCrypto.is_featured === 1
                    ">
                      {{ $t("Pay Via") }}
                      {{ addSiteStore.selectedCrypto.title }}
                      {{ $t("and get") }}
                      <span class="text-[14px] font-[700]">{{ addSiteStore.selectedCrypto.discount }}%</span>
                      {{ $t("discount") }}
                    </div>
                  </div>
                </div>
                ...
                <div class="order-1 mx-[4px]">
                  <input :id="'radio_' + addSiteStore.selectedCrypto.name" type="radio" name="radio" class="hidden"
                    :value="addSiteStore.selectedCrypto.name" @click="" checked number />
                  <label :for="'radio_' + addSiteStore.selectedCrypto.name"
                    class="flex items-center cursor-pointer rtl:pl-[40px] ltr:pr-[40px]">
                    <span
                      class="w-[22px] h-[22px] bg-white dark:bg-tamkinDarkPrimary inline-block mr-1 rounded-full border border-tamkin"></span>
                  </label>
                </div>
              </div>
            </div>
              ...
              <div class="w-full mt-[20px]">
                <div class="relative w-full font-[Poppins]">
                  <button @click="toggleDropdown"
                    ...
                    <span class="floating_label" v-if="!addSiteStore.selectedCrypto">{{ $t("Choose Crypto currency")
                      }}</span>
                      ...
                      <div class="flex items-center justify-end">
                        <div class="rtl:mr-auto ltr:ml-auto">
                          <div class="text-[14px] leading-[24px] font-[500] text-[#878787] dark:text-whiteTamkin">
                            {{
                              cryptostore.rates[
                              addSiteStore.selectedCrypto.coingecko_id
                              ]
                            }}
                          </div>
                        </div>

                        <img v-if="addSiteStore.selectedCrypto" src="/assets/imgs/menu-down.svg" :class="[
            ...
            <div class="flex flex-col items-start justify-center space-y-[24px] w-full px-4 md:px-[20px]"
                  ...
                  <div class="flex items-center justify-start rtl:space-x-reverse space-x-[13px]">
                    <div
                      ...
                      <!-- <span class="text-black font-[500] dark:text-whiteTamkin">
                            {{convertUsdToCryptos(addSiteStore.cartTotal,cryptostore.rates)}} {{selectedCrypto.title}}</span> -->
                      {{
                        convertUsdToCrypto(
                          addSiteStore.packagePayload.total -
                          addSiteStore.currentDiscount,
                          cryptostore.rates,
                          addSiteStore.selectedCrypto.title
                        )
                      }}
            ...
            <div class="flex flex-col items-center justify-center space-y-[12px] mx-auto w-full">
              <div
                ...
                <div class="lg:py-[17px] search_input w-full lg:w-3/4 mt-[24px]">
                  ...
                  <div v-if="
                    addSiteStore.isPromoFilled && !addSiteStore.noDiscount
                  " @click="clearInput"
                ...
                <div class="text-center mt-[24px] w-auto md:w-2/6">
                  <button class="btn-dashboard max-md:!text-[13px] hover_tamkin w-full mx-auto text-center text-nowrap"
                    ...
                    <div class="flex items-center justify-center">
                      ...
                      <svg v-if="addSiteStore.loadingPromo" class="animate-spin h-5 w-5 text-white"
              ...
              <div v-if="addSiteStore.noDiscount"
            ...
            <table class="min-w-full">
              ...
              <tbody>
                <tr class="text-[14px] leading-[24px] font-[500] bg-[#FAFCFE] dark:bg-tamkinDarkPrimary" v-if="false">
                  <td
                  ...
                  <td class="py-2 px-5 border-b dark:border-darkborder text-right w-full dark:text-whiteTamkin"
                    colspan="2">
                    ${{ percentageOff }}
                  </td>
                </tr>
                ...
                <tr v-if="false" class="text-[14px] leading-[24px] font-[500] bg-[#FAFCFE] dark:bg-tamkinDarkPrimary">
                  <td
                    class="py-2 px-5 border-b dark:border-darkborder text-right font-[500] w-full dark:text-whiteTamkin"
                    colspan="2">
                    {{ $t("Total") }}
          ...
          <div v-if="loadingData"
            ...
            <div class="flex flex-col items-center justify-center w-full px-4 md:px-[20px] min-h-full">
              <div
                ...
                <div class="flex items-center justify-between p-[16px]">
                  <div class="flex items-center rtl:space-x-reverse space-x-[13px]">
                    ...
                    <div class="flex flex-col justify-center space-y-[-4px]">
                      ...
                      <div class="w-[100px] h-[14px] bg-gray-300 dark:bg-gray-600 rounded mt-[4px]"></div>
                    </div>
                  </div>
                  <div
                    class="w-[22px] h-[22px] bg-gray-300 dark:bg-gray-600 rounded-full border border-tamkin dark:border-darkborder">
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="w-full mt-[20px]">
                <div class="relative w-full">
                  <button
                    ...
                    <div class="w-[10px] h-[10px] bg-gray-300 dark:bg-gray-600 rounded"></div>
                  </button>
                  <div
          ...
          <div class="mt-[39px] mx-auto mb-[34px] px-4 md:px-[20px] w-full" v-if="!loadingData">
            <button class="btn-dashboard hover_tamkin !h-[40px] w-full" :disabled="!addSiteStore.selectedCrypto" @click="
              navigateTo(
                'crypto_addsite_step1',
                'addSite',
