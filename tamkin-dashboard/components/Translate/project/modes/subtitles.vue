...
const deepEqualTexts = (arr1: any[], arr2: any[]): boolean => {
  return (
    ...
    arr1.every((item, i) => item.text === arr2[i].text)
  );
};
...
<template>
  <div
    v-if="!translateStore.loadingProject"
    class="w-2/4 flex flex-col items-start justify-start h-[315px]"
  >
    <div
      class="flex items-start w-full justify-between rtl:pl-[20px] ltr:pr-[20px]"
      v-if="
        translateStore.currentMode === 'subtitles' && !translateStore.subMode
      "
    >
      ...
      <div
        class="flex items-start justify-evenly rtl:space-x-reverse space-x-[15px]"
      >
        <button
          class="btn-translate tamkin hover_tamkin group"
          @click="changeMode('TranslationStyle')"
          :class="[
            translateStore.subMode === 'TranslationStyle'
              ? 'active_tamkin'
              : '',
          ]"
        >
          <div>
            ...
            >
              <path
                ...
                :class="[
                  translateStore.subMode === 'TranslationStyle'
                    ? 'stroke-[url(#paint0_linear_6767_85371)]'
                    : 'stroke-current',
                ]"
        ...
        <button
          class="btn-translate tamkin hover_tamkin group"
          @click="changeMode('style')"
          :class="[
            translateStore.currentMode === 'style' ? 'active_tamkin' : '',
          ]"
        >
          <div>
            <svg
            ...
            >
              <path
                class="group-hover:stroke-white"
                d="M1.26579 13.2128L5.79073 8.69597M11.4889 6.68741L10.1344 10.7436C10.1127 10.8086 10.0757 10.8674 10.0265 10.9151C9.97721 10.9628 9.91714 10.998 9.85138 11.0175L1.54807 13.481C1.47326 13.5032 1.3938 13.5045 1.31832 13.4847C1.24284 13.4649 1.17424 13.4249 1.11998 13.3689C1.06573 13.3129 1.0279 13.2431 1.0106 13.1672C0.993314 13.0912 0.997225 13.012 1.02191 12.9381L3.71811 4.86551C3.73743 4.80754 3.76902 4.75441 3.81073 4.70971C3.85244 4.66501 3.9033 4.6298 3.95985 4.60645L7.79013 3.03225C7.868 3.00014 7.95364 2.99178 8.03627 3.0082C8.1189 3.02462 8.19481 3.06509 8.25442 3.12451L11.3858 6.25163C11.4418 6.30754 11.4811 6.37793 11.4993 6.4549C11.5175 6.53187 11.5139 6.61238 11.4889 6.68741Z"
                :class="[
                  translateStore.currentMode === 'style'
                    ? 'stroke-white'
                    : 'stroke-current',
                ]"
                stroke-linecap="round"
              ...
              <path
                class="group-hover:stroke-white"
                d="M11.5731 6.43703L13.5839 4.42917C13.8505 4.16298 14.0002 3.802 14.0002 3.4256C14.0002 3.0492 13.8505 2.68822 13.5839 2.42203L12.0765 0.915959C11.9445 0.784088 11.7877 0.679478 11.6151 0.608107C11.4425 0.536735 11.2576 0.5 11.0708 0.5C10.884 0.5 10.699 0.536735 10.5265 0.608107C10.3539 0.679478 10.1971 0.784088 10.065 0.915959L8.05428 2.92381M7.29918 7.18935C7.20011 7.09051 7.0825 7.01211 6.95308 6.95863C6.82366 6.90516 6.68496 6.87765 6.54494 6.87765C6.40493 6.87765 6.26622 6.90516 6.1368 6.95863C6.00738 7.01211 5.88977 7.09051 5.79073 7.18935C5.69188 7.2882 5.61336 7.40557 5.55977 7.53488C5.50617 7.66419 5.47855 7.80293 5.47852 7.94341C5.47848 8.08339 5.50605 8.22213 5.55964 8.35144C5.61324 8.48075 5.69176 8.59812 5.79073 8.69698"
                :class="[
                  translateStore.currentMode === 'style'
                    ? 'stroke-white'
                    : 'stroke-current',
      ...
      <div class="mt-[16px] w-full">
        <div class="space-y-2 flex flex-col items-start justify-center">
          <div
            ...
            :key="textItem.idx"
            class="flex items-center justify-between border-b py-2 w-full rtl:pl-[8px] ltr:pr-[8px] relative"
          >
            <button
            ...
            >
              <div
                v-if="!texts.find((it) => it.id === textItem.idx)?.isEditing"
              >
                <p
            ...
            <div
              class="flex items-center justify-evenly rtl:space-x-reverse space-x-[20px] ipad-max:w-[45%] w-[40%] 3xl:w-[30%]"
            >
              <div class="flex flex-col">
                <div class="flex items-center rtl:space-x-reverse space-x-4">
                  <img
                    src="/assets/imgs/translatevideo/in_watch.png"
                    class="w-[12px] h-[14px]"
                    alt=""
                  />
                  <div class="flex items-center rtl:space-x-reverse space-x-2">
                    <span
                      class="text-[12px] leading-[32px] font-[400] text-[#878787]"
                      >{{ $t("In") }}</span
                    >
                    <span
                      class="text-[12px] leading-[32px] font-[400] text-[#878787]"
                      >{{ formatTimeString(textItem.start_time) }}</span
                    >
                  </div>
                </div>
                <div class="flex items-center rtl:space-x-reverse space-x-4">
                  <img
                    src="/assets/imgs/translatevideo/out_watch.png"
                    class="w-[12px] h-[14px]"
                    alt=""
                  />
                  <div class="flex items-center rtl:space-x-reverse space-x-2">
                    ...
                    <span
                      class="text-[12px] leading-[32px] font-[400] text-[#878787]"
                      >{{ formatTimeString(textItem.end_time) }}</span
                    >
                  </div>
                </div>
              </div>
              <button
                @click="deleteSub(textItem.idx)"
    ...
    <transition name="slide-up">
      <SaveTranslateFooter
        :show-footer="donePros"
        @save="updateProjectFn"
  ...
  <div
    v-else
    class="w-2/4 flex flex-col items-start justify-start h-[315px] space-y-4 animate-pulse"
  >
    <!-- Header placeholder -->
    <div
      class="flex items-start w-full justify-between rtl:pl-[20px] ltr:pr-[20px]"
    ...
    <div class="w-full h-[275px]">
      ...
      >
        ...
        <div
          class="absolute top-[-10px] rtl:right-[30%] ltr:left-[30%] bg-gray-300 h-[20px] rounded-[5px] w-[10px]"
        ></div>
        ...
        <div
          class="flex items-center justify-evenly rtl:space-x-reverse space-x-[20px] w-[40%]"
        >
