...
<template>
  ...
  <!-- Large Screen -->
  <div v-if="
      currentCategoryWithSkinItems.skin_items_list.length > 0 && !loadingChars && windowWidth >= 768
    "
    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 overflow-x-hidden dark:bg-tamkinDarkPrimary bg-white pt-4 !pb-4 px-[15px] rounded-b-[10px] lg:gap-2 2xl:gap-2 ipad-max:gap-8 relative z-[10]">
    <!-- @click.stop="marketStore.selectItemforPreview(skin_item)" -->
    <div class="market_card_char order-1 cursor-pointer" @click="playerStore.wearClothes(skin_item)"
      :role="marketStore.owned(skin_item) ? 'button' : ''"
      v-for="skin_item in currentCategoryWithSkinItems.skin_items_list" :key="skin_item.name" :class="[
      ...
      <div :class="[
        ...
        <div class="h-[120px] flex items-center justify-center">
          <img :src="fullUrl(skin_item.image)" v-if="skin_item.category !== 'Background'" class="w-[78px] h-[78px]"
            :alt="skin_item.text" />
          <div v-if="
            skin_item.category === 'Background' &&
            skin_item.background_color !== null
          " class="w-[78px] h-[78px] rounded-[10px]" :style="{ background: skin_item.background_color }"></div>
          <img :src="fullUrl(skin_item.background_image)" v-if="
            skin_item.category === 'Background' &&
            skin_item.background_image !== null
          " class="w-[78px] h-[78px] rounded-[10px]" :alt="skin_item.text" />
        </div>
        <div
          class="absolute top-0 left-0 w-[51px] h-[17px] rounded-tl-[10px] flex items-center text-[#021328] justify-center"
          v-if="skin_item.is_weared" style="
        ...
        <div v-else-if="skin_item.is_purchased"
        ...
        <div v-else-if="skin_item.is_pending"
      ...
      <div class="flex flex-col justify-center items-evenly w-full space-y-[10px] p-1">
        ...
        <div class="flex flex-col !mt-[16px]">
          <div v-if="marketStore.cartable(skin_item) && skin_item.offer_cost > 0"
          ...
          <div class="flex items-end justify-between w-full" v-if="
  ...
  <!-- Small Screen -->
  <Splide v-if ="currentCategoryWithSkinItems.skin_items_list.length > 0 && !loadingChars && windowWidth < 768" :options="{
    rewind: true,
    arrows: false,
    ...
    <SplideSlide v-for="skin_item in currentCategoryWithSkinItems.skin_items_list" :key="skin_item.name">
      <div class="market_card_char order-1 cursor-pointer" @click="playerStore.wearClothes(skin_item)"
      :role="marketStore.owned(skin_item) ? 'button' : ''" :class="[
        playerStore.lastClickedSkinItemName == skin_item.name
          ? 'bg-selected dark:bg-p custom-border-tamkin padding-override-1'
          : '',
      ]">
  ...
  <div v-if="loadingChars"
    class="grid dark:bg-tamkinDarkPrimary grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4` overflow-x-hidden bg-white pt-4 !pb-4 px-[15px] rounded-b-[10px] lg:gap-2 2xl:gap-2 ipad-max:gap-8 relative z-[10]">
