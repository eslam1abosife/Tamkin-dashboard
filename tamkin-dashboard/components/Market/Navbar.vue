...
const categoriesHavingSkinItems = computed(() => {
  return props.categoriesWithSkinItems && props.categoriesWithSkinItems.length > 0
    ? props.categoriesWithSkinItems.filter(
        (category) => category.skin_items_list.length > 0
      )
    : [];
});
...
<template>
  ...
  >
    <div
      class="w-[200px] flex items-center justify-between rtl:space-x-reverse space-x-2"
    >
      <div
        ref="character"
        @click="switchTabAndScroll('character')"
        :class="[
          marketStore.currentTab === 'character' ? 'bg-white dark:bg-[#344153]' : '',
        ]"
            ...
            <path
              ...
              :class="
                marketStore.currentTab === 'character'
                  ? 'stroke-[#2DB9B0]'
                  : 'stroke-[#878787] dark:stroke-whiteTamkin/60'
              "
      ...
      <!-- dynamic -->
      <div
        v-for="category in categoriesHavingSkinItems"
        :key="category.name"
        :ref="
          (el) => {
            if (el) {
        ...
        :class="[
          marketStore.currentTab === category.text
