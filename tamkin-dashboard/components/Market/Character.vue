...
const { isOpen, openModal, setData } = useModalManager();
...
<template>
  ...
  <!-- Large Screen -->
  <div
    v-if="!loadingChars && windowWidth >= 768"
              ...
              >
                ...
                >
                  <div>
                    %{{
                      (
                        ((char.cost - char.offer_cost) / char.cost) *
                        100
                      ).toFixed(2)
                    }}
                    {{ $t("OFF") }}
  ...
  >
    <div
      v-for="i in windowWidth >= 1024 ? 5 : windowWidth >= 768 ? 4 : 2"
      :key="i"
