...
const checkAll = computed({
  get() {
    return (
      permissions.value && checked.value.length === permissions.value.length
    );
  },
  set(value) {
...
const activePermissions = computed(() => {
  return permissions.value.filter(
    (permission) => permission.status !== "deleted"
  );
});
...
<template>
  ...
  >
    ...
    <div class="container mx-auto max-h-auto">
      <div v-loading="getAppsLoading">
        ...
        <div
          class="flex items-center rtl:space-x-reverse space-x-[12px] justify-start ipad-max:mt-0 mt-[32px] border-[1px] border-t border-b-0 border-l-0 border-r-0 pt-[16px]"
        >
          <!-- Avatar Placeholder -->
          <div
          ...
          <div class="flex flex-col items-start justify-center">
            <div v-if="!getAppsLoading">
              <h2
                class="ltr:text-left rtl:text-right font-[500] text-darkGrey dark:text-whiteTamkin text-[14px]"
              >
            ...
            <div
              v-else
              class="h-4 w-2/3 bg-gray-300 dark:bg-gray-600 rounded animate-pulse"
            ></div>
            ...
            <div
              v-else
              class="mt-2 h-3 w-1/2 bg-gray-300 dark:bg-gray-600 rounded animate-pulse"
            ></div>
          </div>
        ...
        >
          {{ $t("Select Website that") }}
          <span class="font-[700] text-darkGrey dark:text-whiteTamkin/60">
            {{ getData().firstName + " " + getData().lastName }}
          </span>
          {{ $t("can access") }}
        </p>
        <div
          class="my-[16px] w-full h-[24px] rounded-[5px] bg-gray-300 dark:bg-gray-600 animate-pulse"
          v-else
        ></div>
        ...
        <div
          class="w-full py-[17px] h-[45px] rounded-[5px] bg-gray-300 dark:bg-gray-600 animate-pulse"
          v-else
        ></div>
      </div>
      ...
      <div v-loading="getAppsLoading" class="overflow-y-auto h-[150px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-darkborder">
            ...
            >
              <td class="py-4 flex items-center rtl:space-x-reverse space-x-4">
                ...
                <span
                  class="text-[14px] leading-[21px] font-[400] text-gray-900 dark:text-whiteTamkin"
                >
                  {{ $t(permission.title) }}
                </span>
              </td>
        ...
        <div
          v-else-if="getAppsLoading"
          class="min-w-full divide-y divide-gray-200 dark:divide-light animate-pulse"
        >
          <table class="min-w-full divide-y divide-gray-200 dark:divide-light">
            <thead>
              <tr>
                ...
                >
                  ...
                  ></div>
                  <div
                    class="w-5 h-5 bg-gray-300 dark:bg-gray-600 rounded"
                  ></div>
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-darkborder">
              <tr v-for="n in 1" :key="n">
                ...
                >
                  <div
                    class="w-6 h-6 bg-gray-300 dark:bg-gray-600 rounded-full"
                  ></div>
                  <span
                    class="block h-4 w-1/3 bg-gray-300 dark:bg-gray-600 rounded"
                  ></span>
                </td>
                <td class="py-4 text-right">
                  <div
                    class="w-5 h-5 bg-gray-300 dark:bg-gray-600 rounded rtl:mr-auto ltr:ml-auto"
                  ></div>
                </td>
              </tr>
            </tbody>
      ...
      <h6
        v-if="errMsg"
        class="text-center text-[red] font-light text-[14px] mt-[5px] !mb-[5px]"
      >
        {{ errMsg }}
      </h6>
