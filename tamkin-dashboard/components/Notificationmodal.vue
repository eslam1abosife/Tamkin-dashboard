...
<template>
  ...
  >
    <div
      ...
      class="fixed top-[0] z-[9999] rtl:left-[0] ltr:right-[0%] drop-shadow-xl bg-white ltr:!rounded-r-[0] rtl:rounded-l-0 dark:bg-tamkinDarkPrimary rtl:rounded-r-[10px] ltr:rounded-tl-[10px] h-full max-h-[100vh] w-[310px] rounded-b-[10px] flex flex-col items-start justify-start"
    >
      <div
        class="h-[69px] dark:bg-p bg-[#35C0B4] rtl:rounded-l-[0] ltr:!rounded-r-[0] p-[20px] flex items-center justify-between rounded-t-[10px] w-full"
      >
        <div class="flex items-center justify-start text-white">
          {{ $t("Notifications") }} ({{
            notificationBellStore.notifications.length
          }})
        </div>
        <div
          @click="closeMenu"
          class="bg-white h-[24px] w-[24px] rounded-full flex items-center justify-center cursor-pointer hover:bg-gradient-to-r from-tamkinStart to-tamkinEnd group"
      ...
      <div
        ...
        v-if="notificationBellStore.notifications.length === 0"
      >
        <div>
          <img
      ...
      >
        <div
          ...
          class="flex items-start rtl:space-x-reverse space-x-4 p-4 cursor-pointer hover:dark:bg-p hover:bg-tamkinLight w-full last:rounded-b-[10px]"
        >
          <!-- Indicator -->
          <div
            class="w-3 h-3 rounded-full mt-1"
            :class="
              notification.status === 'Sent' ? 'bg-[#35C0B4]' : 'bg-[#A7A7A7]'
            "
          ></div>
          ...
          <div
            class="flex-1 w-full"
            @click="notificationSelected(notification.name)"
          >
            <p
              :class="
