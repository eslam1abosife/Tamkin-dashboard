...
const selectedPackage = ref(
  mysiteStore.currentPackage.billing_duration === "3-monthly" ||
...
const removeWebsite = (website: any) => {
  mysiteStore.urls = mysiteStore.urls.filter(
...
const calculatePrice = (website) => {
  ...
  if (pricebytraffic.value.length > 0) {
    const currentWebsite = mysiteStore.urls.find(
      (item) => item.url === website
    );
...
watch(mysiteStore.urls, async () => {
  if (levelof.value && levelof.value === "Over 1M page views/mo") {
    const res = await getPriceByTraffic(
      [
        ...(mysiteStore.urls.length
          ? mysiteStore.urls.map((we) => we.url)
          : []),
      ],
...
const checkforexistingwebsite = () => {
  ...
  if (
    apps.value.filter((website: any) => website.app_domain === cleanDomain)
      .length > 0 ||
    mysiteStore.urls.filter((website: any) => website.url === cleanDomain)
      .length > 0
  ) {
    websiteExist.value = true;
  } else {
              ...
              >
                <div class="flex items-center justify-center w-full">
                  <div class="order-2 w-3/4 h-full">
                    ...
                    <h2 class="font-[500] text-[10px] dark:text-whiteTamkin">
                      {{ $t("Free trial for") }}
                      {{
                  ...
                  <div
                    v-if="
                      mysiteStore.currentPackage.billing_duration ===
                        'monthly' && !mysiteStore.currentPackage.cancel_package
                    "
                    class="bg-gradient-to-br from-yellow-600 to-yellow-300 absolute text-[13px] font-[400] w-[80px] rounded-[10px] h-[22px] flex items-center justify-center px-[0.5px] top-[-10px] left-[calc(50%-45px)] text-white"
                  >
                    <span>{{ $t("Renew") }}</span>
                  </div>
                  <div class="order-2 w-3/4 h-full">
              ...
              <div class="flex items-center justify-center w-full">
                <div class="order-2 w-3/4 h-full">
                  <div
                    v-if="
                      mysiteStore.currentPackage.billing_duration ===
                        '3 months' && !mysiteStore.currentPackage.cancel_package
                    "
              ...
              <div class="flex items-center justify-center w-full">
                <div class="order-2 w-3/4 h-full">
                  <div
                    v-if="
                      mysiteStore.currentPackage.billing_duration ===
                        'yearly' && !mysiteStore.currentPackage.cancel_package
                    "
                    class="bg-gradient-to-br from-yellow-600 to-yellow-300 absolute text-[13px] leading-[17.76px] font-[400] w-[80px] rounded-[10px] h-[22px] flex items-center justify-center py-[4.5] px-[0.5px] top-[-10px] left-[calc(50%-40px)] text-white"
                  >
                    <span>{{ $t("Renew") }}</span>
