    ...
    <div class="grid grid-cols-3 gap-[10px] w-full">
      ...
      >
        <div
          ...
          :class="[
            pak.type_deal === 'Best Value' ? 'bg-[#C16487]' :
            pak.type_deal === 'Best Deal' ? 'bg-gradient-to-br from-[#2dada3] to-[#71dad2]' :
            'bg-[#5E4352]'
          ]"
        ...
        <div class="flex items-center justify-center w-full px-[15px] mt-[48px]">
          <div class="order-2 w-full">
            ...
            <h2 class="font-[400] h-[30px] text-[14px] leading-[15px] text-[#536174] dark:text-whiteTamkin">
              {{ $t(pak.sub_title) }}
            </h2>

            <h3
              v-if="pak.package_price_role[0].cost_month > 0 || pak.package_price_role[0].cost_yearly > 0 || pak.cost_investor === 0"
            ...
            <h3 v-else-if="pak.type !== 'Investors'" class="mt-[16px] text-black dark:text-whiteTamkin font-[600] text-[24px] leading-[29px]">
              {{ $t('Free') }}
            </h3>

            <div v-if="pak.package_price_role[0].cost_before_month !== 0 || pak.package_price_role[0].cost_before_yearly !== 0" class="text-[#EA4335] text-[16px] font-[500] line-through px-[2px]">
            ...
            <p v-if="pak.type !== 'Investors'" class="font-[700] text-[10px] leading-[32px] text-darkGrey dark:text-whiteTamkin">
              {{ $t(pak.description) }}
            </p>

            <div v-else-if="pak.cost_investor > 0">
              <div>$ {{ pak.cost_investor.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</div>
              <h2 class="font-[500] my-[16px] text-[14px] leading-[15px] text-[#536174] dark:text-whiteTamkin">
        ...
        <!-- Package Items -->
        <div class="flex group-hover:bg-selected dark:group-hover:bg-p dark:text-whiteTamkin flex-col items-start justify-center w-full space-y-[10px] h-full custom-border-collapse rounded-t-none rounded-[10px] p-4">
          <div
            v-for="item in pak.package_items.filter((k) => k.section === 'Package').sort((a, b) => a.idx - b.idx)"
            :key="item.name"
            class="flex items-center justify-start rtl:space-x-reverse space-x-[24px]"
          >
            <img class="w-[16px] h-[16px]" :src="item.is_available ? '/assets/imgs/checked_list_active.svg' : '/assets/imgs/checked_list_inactive.svg'" />
            <h3 class="text-[12px] font-[400] leading-[20px]">{{ $t(item.title) }}</h3>
          </div>
          ...
          <div class="flex items-center justify-center mx-auto w-full">
            <button
              ...
              :disabled="pak.cost_month !== 0 && pak.cost_yearly !== 0 || pak.title === 'Free' || (currentPackage?.type === 'Investors' && mySiteStore.currentPackage?.name === pak.name)"
              class="btn-dashboard hover_tamkin w-full !rounded-[19px] mx-auto"
            >
              <!-- Button Text Logic -->
              <span v-if="pak.is_contact_us"> {{ $t('Contact us') }} </span>
              <span v-else-if="!isLinkActive('/overview') && currentPackage?.name === pak.name && currentPackage?.status !== 'Rejected' && mySiteStore.currentWebsite?.status !== 'Rejected' && currentPackage?.status !== 'Cancelled' && pak.title !== 'Free'"> {{ currentPackage?.type !== 'Investors' ? $t('Renew') : $t('Current Package') }} </span>
              <span v-else-if="currentPackage?.name === pak.name && (currentPackage?.status === 'Rejected' || currentPackage?.status === 'Cancelled')"> {{ $t('Try Again') }} </span>
              <span v-else-if="pak.trial_days > 0 || (pak.trial_days === 0 && pak.cost_month !== 0 && pak.cost_yearly !== 0)"> {{ pak.trial_days > 0 ? $t('Free Trial') : '' }} </span>
              <span v-else-if="(pak.cost_month === 0 && pak.cost_yearly === 0) || pak.title === 'Free'"> {{ $t('Free Package') }} </span>
