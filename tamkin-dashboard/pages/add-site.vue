...
const handleChangeTag = (tags: any) => {
  ...
  if (tags.length > previousTagCount) {
    ...
    const alreadyValidated = validatedSites.value.some(
      (site) => site.domain === newTag
    );
    if (!alreadyValidated) {
...
const checkifSiteBlockedOrNot = async (site) => {
  ...
  if (siteInApps) {
    $toast("One of the websites is already included in your list of sites.", {
      hideIn: 3000,
      type: "warning",
    });

    // If site exists in apps, mark it as not blocked and push to validatedSites
    validatedSites.value.push({
      domain: cleanUrl,
      blocked: false,
      title: cleanUrl,
      exists: true,
...
const addSitesAndOpenModal = async () => {
  ...
  try {
    ...
    const trafficPromises = validatedSites.value
      ...
      .map(async (site) => {
        const traffic =
          selectedPlan.value.type === "Accessibility"
            ? await getTraffic(site.domain, "url")
            : null;
        return {
          ...
          traffic:
            selectedPlan.value.type === "Accessibility"
              ? determineTrafficLevel(traffic[0].traffic)
              : null,
        };
      });
...
const sortedPlans = computed(() => {
  ...
  const specificTypePackages = addSiteStore.packages
    .filter((pkg) => pkg.type === desiredType)
    .slice(0, 3);

  const otherPackages = addSiteStore.packages
    .filter((pkg) => pkg.type !== desiredType)
    .slice(0, 3);
...
<template>
  <div class="w-full" v-if="!loadingPage">
    ...
    <div class="space-y-[10px]">
      <h1
        class="rtl:text-right ltr:text-left text-[18px] leading-[36px] font-[600] dark:text-whiteTamkin"
      >
        {{ $t("Add Site") }}
      </h1>
      ...
      >
        <template #item="{ name, index }">
          ...
          >
            ...
            <svg
              v-if="
                loadingBlock.includes(cleanWebsiteUrl(name)) || !siteg(name)
              "
              class="animate-spin h-3 w-3 text-tamkin"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
            >
            ...
            <img
              ...
              v-if="
                !loadingBlock.includes(cleanWebsiteUrl(name)) &&
                siteg(name) &&
                !siteg(name).blocked &&
                !siteg(name).exists
              "
 ...
 <div class="mt-[16px] w-full h-[164px]  bg-transparent focus:outline-none focus:ring-0 
  ...
  <div class="overflow-x-hidden animate-pulse" v-else>
    <div class="space-y-[10px]">
      <div
        class="w-[150px] h-[30px] bg-gray-300 dark:bg-gray-700 rounded"
      ></div>
      <div
        class="w-[300px] h-[20px] bg-gray-300 dark:bg-gray-700 rounded"
      ></div>
    </div>
    ...
    <div class="mt-[32px] flex items-center justify-between">
      ...
      <div class="flex items-center space-x-[10px] rtl:space-x-reverse">
        <div
          class="w-[100px] h-[24px] bg-gray-300 dark:bg-gray-700 rounded"
        ></div>
        <div
          class="w-[12px] h-[12px] bg-gray-300 dark:bg-gray-700 rounded-full"
        ></div>
      </div>
    </div>
    ...
    <div
      class="mt-[35px] flex space-x-[16px] rtl:space-x-reverse overflow-hidden"
    >
      <!-- Add more placeholder divs to simulate multiple package cards -->
      <div
        class="relative w-11/12 py-[62px] h-[149px] bg-gray-300 dark:bg-gray-700 rounded-[10px]"
        v-for="(f, i) in 2"
        :key="i"
      >
        <div class="mt-[22px]">
          <div
            class="w-[75%] h-[16px] bg-gray-300 dark:bg-gray-700 mt-[10px] rounded"
          ></div>
        </div>
      </div>
    </div>
    ...
    <div
      class="mt-[16px] w-full h-[164px] bg-gray-300 dark:bg-gray-700 rounded-[10px]"
    ></div>
