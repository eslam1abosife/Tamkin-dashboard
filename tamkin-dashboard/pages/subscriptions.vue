...
<template>
  <div class="w-full mx-auto">
    ...
    <ModalsConfirm :show-modal="true" v-if="isOpen('cancel_subscription_subs')" title="Cancel Subscription"
      sub-title="Are you sure you want to Cancel the subscription?" confirm-btn-type="delete"
      deleteButtonName="Cancel Subscription" @control-delete="cancelSubscriptionInternal"
    ...
    " style="box-shadow: 0px 4px 12px 0px #00000014"
      ...
      <div class="flex flex-col items-start justify-start static z-[40]">
        ...
        <div class="text-[13px] font-[500] leading-[19px] text-darkGrey dark:text-white dark:drop-shadow-xl mt-[6px]">
    ...
    <div v-else-if="loadingRenews"
      class="w-full h-[125px] dark:bg-tamkinDarkPrimary bg-gray-300 animate-pulse my-[30px] rounded-[10px]"></div>

    <div class="bg-white dark:bg-tamkinDarkPrimary w-full h-full mt-[32px] rounded-[10px] p-[32px]" v-if="
      subs.filter((t) => t.subscripitions.length > 0).length === 0 &&
      !loadingSubs
    ">
      <div class="flex flex-col items-center justify-center space-y-[10px]">
        <img src="/imgs/no_subs.png" class="w-[71px] h-[71px]" alt="" />
        <div class="text-[14px] leading-[28px] font-[400] text-darkGrey dark:text-white w-f md:w-1/4 text-center">
          {{
            $t(`You don't have any subscriptions in this list at the moment`)
          }}
        </div>

        <button @click="$router.push(localePath('/packages'))" class="btn-dashboard hover_tamkin w-[158px]">
          {{ $t("Subscribe Now") }}
        </button>
      </div>
    </div>
    <div v-else-if="loadingSubs"
      class="bg-gray-300 dark:bg-tamkinDarkPrimary rounded-[10px] h-[273px] w-full animate-pulse"></div>
    <div v-if="
      subs.filter((t) => t.subscripitions.length > 0).length > 0 &&
      !loadingSubs
    " style="box-shadow: 0px 4px 12px 0px #00000014"
      class="bg-white dark:bg-tamkinDarkPrimary rounded-[10px] w-full h-full px-[16px] py-[28px]">
      <div class="flex flex-col md:flex-row items-center md:justify-between w-full space-x-[24px] rtl:space-x-reverse">
        <div class="lg:w-[80%] w-full">
          <div class="py-[17px] search_input">
            <input type="text" class="input_dashboard_search lg:w-[80%] w-full" v-model="search"
        ...
        <div
          ...
          <div @click="changePeriod(0)" :class="[
      ...
      <div v-if="!loadingSubs">
        <div v-for="(sub, index) in filteredSubs" :key="index">
          <div class="flex items-center justify-start rtl:space-x-reverse space-x-[10px] mt-[42px]">
            <div>
              <img src="/assets/imgs/icons/mysite_select.svg" class="w-[30px] h-[30px]"
              ...
              <div v-if="sub?.favicon && sub?.title !== 'Internal Service'">
                <img v-if="sub.favicon" :src="sub.favicon"
            ...
            <div class="">
              <table class="table-auto w-full text-sm rounded-[10px] bg-[#F5F9FF] dark:bg-darkGrey">
                ...
                <tbody class="bg-[#F5F9FF] dark:bg-darkGrey">
                  <tr v-for="sb in sub.subscripitions.filter((f) => {
                    ...
                    <td
                      ...
                      <div class="flex items-center space-x-2 rtl:space-x-reverse">
                        <img :src="configrun.public.baseImagerUrl + sb.package_icon
                          " class="w-[20px] h-[20px] object-cover rounded-full overflow-hidden" alt="Image" />
                    ...
                    <td :class="sb.status === 'active' || sb.status === 'Active'
                        ? 'text-tamkin '
                    ...
                    <td
                      ...
                      <div
                        ...
                        <button :disabled="sb.package_type === 'Addons' ||
                          ...
                          " class="animate-spin h-5 w-5 text-tamkin" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24">
                          ...
                          <svg v-else width="18" height="18" viewBox="0 0 18 18" class="size-5" fill="none"
                        ...
                        <button :disabled="sb.package_type === 'Extra' ||
                          ...
                          <svg v-else width="18" height="18" viewBox="0 0 18 18" class="size-5" fill="none"
                            ...
                            <defs>
                              <pattern id="pattern0_11966_4756" patternContentUnits="objectBoundingBox" width="1"
                                height="1">
                                <use xlink:href="#image0_11966_4756"
                                  transform="translate(-0.132812 -0.132812) scale(0.0140625)" />
                              </pattern>
                              <image id="image0_11966_4756" width="90" height="90"
                        ...
                        <button :disabled="sb.package_type === 'Extra' ||
                          ...
                          @click="() => {
                            ...
                            ">
                          <svg width="19" height="19" viewBox="0 0 19 19" class="size-[20px]" fill="none"
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                            <rect width="19" height="19" fill="url(#pattern0_11966_4757)" />
                            <defs>
                              <pattern id="pattern0_11966_4757" patternContentUnits="objectBoundingBox" width="1"
                                height="1">
                                <use xlink:href="#image0_11966_4757"
                                  transform="translate(-0.0625 -0.0625) scale(0.0125)" />
                              </pattern>
                              <image id="image0_11966_4757" width="90" height="90"
                ...
                <tr v-for="index in 2" :key="index">
                  <td
                    ...
                    <div class="flex items-center justify-start rtl:space-x-reverse space-x-[10px]">
                      <div class="w-[30px] h-[30px] bg-gray-300 dark:bg-gray-600 rounded-full animate-pulse"></div>
                      <div class="w-[150px] h-[20px] bg-gray-300 dark:bg-gray-600 rounded animate-pulse"></div>
                    </div>
                  </td>
                  <td
