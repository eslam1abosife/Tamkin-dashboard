...
const getCurrentPackageToUpgrade = computed(()=>{
  ...
  return !loadingPage.value  ? mySiteStore.currentWebsite.package.find(pck => pck.name === route.query.package) : ''
})
      ...
      <div class="flex flex-col items-start justify-center w-full px-[15px]">
        ...
        >
          <div class=" flex items-center justify-start rtl:space-x-reverse space-x-[4px] ">
            <div>
           <img  :src="runtimeConfig.public.baseImagerUrl + (getCurrentPackageToUpgrade ? getCurrentPackageToUpgrade.icon : '/')"  class="lg:w-[22px] lg:h-[22px] w-[10px] h-[10px] " />
            </div>
...
ipad-max:leading-[10px] whitespace-nowrap
  ...
  {{getCurrentPackageToUpgrade.title}} - {{ currentApp.app_domain !== null ? currentApp.app_domain : 'Internal Service'}}
          ...
          <div
          v-if="new Date() > new Date(getCurrentPackageToUpgrade.endpackage)"
        ...
        <div
        v-if="getCurrentPackageToUpgrade.status === 'Rejected' || getCurrentPackageToUpgrade.status === 'Cancelled'"
      ...
      >
        ...
        >
          {{ getCurrentPackageToUpgrade.status === 'Pending'? $t('Under Review') : $t(`${getCurrentPackageToUpgrade.status}`) }}
        </div>
  ...
  <div    class="mt-[24px] bg-white dark:bg-tamkinDarkPrimary rounded-[10px] h-full shadow-md -shadow-y-[1px]" v-if="loadingPage" >
